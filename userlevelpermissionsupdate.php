...
$userlevelpermissions_update = NULL; // Initialize page object first

class cuserlevelpermissions_update extends cuserlevelpermissions {
	...
	// Page object name
	var $PageObjName = 'userlevelpermissions_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("userlevelpermissionslist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->_tablename->setDbValue($this->Recordset->fields('tablename'));
					$this->permission->setDbValue($this->Recordset->fields('permission'));
				} else {
					if (!ew_CompareValue($this->userlevelid->DbValue, $this->Recordset->fields('userlevelid')))
						$this->userlevelid->CurrentValue = NULL;
					if (!ew_CompareValue($this->_tablename->DbValue, $this->Recordset->fields('tablename')))
						$this->_tablename->CurrentValue = NULL;
					if (!ew_CompareValue($this->permission->DbValue, $this->Recordset->fields('permission')))
	...
	function SetupKeyValues($key) {
		...
		$sKeyFld = $key[1];
		$this->_tablename->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->_tablename->MultiUpdate = $objForm->GetValue("u__tablename");
		if (!$this->permission->FldIsDetailKey) {
			$this->permission->setFormValue($objForm->GetValue("x_permission"));
		}
		$this->permission->MultiUpdate = $objForm->GetValue("u_permission");
	}
	...
	function ValidateForm() {
		...
		if ($this->_tablename->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->permission->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if ($this->permission->MultiUpdate <> "" && !$this->permission->FldIsDetailKey && !is_null($this->permission->FormValue) && $this->permission->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->permission->FldCaption(), $this->permission->ReqErrMsg));
		}
		if ($this->permission->MultiUpdate <> "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->permission->SetDbValueDef($rsnew, $this->permission->CurrentValue, 0, $this->permission->ReadOnly || $this->permission->MultiUpdate <> "1");
...
// Create page object
if (!isset($userlevelpermissions_update)) $userlevelpermissions_update = new cuserlevelpermissions_update();

// Page init
$userlevelpermissions_update->Page_Init();

// Page main
$userlevelpermissions_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$userlevelpermissions_update->Page_Render();
?>
...
var CurrentForm = fuserlevelpermissionsupdate = new ew_Form("fuserlevelpermissionsupdate", "update");
...
fuserlevelpermissionsupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_permission");
...
fuserlevelpermissionsupdate.Form_CustomValidate = 
...
fuserlevelpermissionsupdate.ValidateRequired = true;
<?php } else { ?>
fuserlevelpermissionsupdate.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$userlevelpermissions_update->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $userlevelpermissions_update->ShowPageHeader(); ?>
<?php
$userlevelpermissions_update->ShowMessage();
?>
<form name="fuserlevelpermissionsupdate" id="fuserlevelpermissionsupdate" class="<?php echo $userlevelpermissions_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($userlevelpermissions_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $userlevelpermissions_update->Token ?>">
<?php } ?>
...
<?php } ?>
<?php foreach ($userlevelpermissions_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_userlevelpermissionsupdate">
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($userlevelpermissions->permission->Visible) { // permission ?>
	<div id="r_permission" class="form-group">
		<label for="x_permission" class="col-sm-2 control-label">
<input type="checkbox" name="u_permission" id="u_permission" value="1"<?php echo ($userlevelpermissions->permission->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $userlevelpermissions->permission->FldCaption() ?></label>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $userlevelpermissions_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div>
	</div>
<?php } ?>
</div>
...
<script type="text/javascript">
fuserlevelpermissionsupdate.Init();
</script>
<?php
$userlevelpermissions_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$userlevelpermissions_update->Page_Terminate();
?>
