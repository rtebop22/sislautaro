...
$zonas_add = NULL; // Initialize page object first

class czonas_add extends czonas {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["Id_Zona"] != "") {
				...
				$this->setKey("Id_Zona", $this->Id_Zona->CurrentValue); // Set up key
			} else {
				$this->setKey("Id_Zona", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("zonaslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "zonaslist.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to list page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "zonasview.php")
...
// Create page object
if (!isset($zonas_add)) $zonas_add = new czonas_add();
...
var CurrentForm = fzonasadd = new ew_Form("fzonasadd", "add");

// Validate form
fzonasadd.Validate = function() {
...
fzonasadd.ValidateRequired = true;
<?php } else { ?>
fzonasadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $zonas_add->ShowPageHeader(); ?>
<?php
$zonas_add->ShowMessage();
?>
<form name="fzonasadd" id="fzonasadd" class="<?php echo $zonas_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($zonas_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $zonas_add->Token ?>">
<?php } ?>
...
<?php if ($zonas->Descripcion->Visible) { // Descripcion ?>
	<div id="r_Descripcion" class="form-group">
		<label id="elh_zonas_Descripcion" for="x_Descripcion" class="col-sm-2 control-label ewLabel"><?php echo $zonas->Descripcion->FldCaption() ?></label>
...
<?php if ($zonas->Id_Departamento->Visible) { // Id_Departamento ?>
	<div id="r_Id_Departamento" class="form-group">
		<label id="elh_zonas_Id_Departamento" for="x_Id_Departamento" class="col-sm-2 control-label ewLabel"><?php echo $zonas->Id_Departamento->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
...
<?php if ($zonas->Fecha_Actualizacion->Visible) { // Fecha_Actualizacion ?>
	<div id="r_Fecha_Actualizacion" class="form-group">
		<label id="elh_zonas_Fecha_Actualizacion" for="x_Fecha_Actualizacion" class="col-sm-2 control-label ewLabel"><?php echo $zonas->Fecha_Actualizacion->FldCaption() ?></label>
...
<?php if ($zonas->Usuario->Visible) { // Usuario ?>
	<div id="r_Usuario" class="form-group">
		<label id="elh_zonas_Usuario" for="x_Usuario" class="col-sm-2 control-label ewLabel"><?php echo $zonas->Usuario->FldCaption() ?></label>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $zonas_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
