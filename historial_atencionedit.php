...
$historial_atencion_edit = NULL; // Initialize page object first

class chistorial_atencion_edit extends chistorial_atencion {
	...
	// Page object name
	var $PageObjName = 'historial_atencion_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->Id_Historial->CurrentValue == "") {
			$this->Page_Terminate("historial_atencionlist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("historial_atencionlist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "historial_atencionlist.php")
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->Id_Historial->FldIsDetailKey)
...
// Create page object
if (!isset($historial_atencion_edit)) $historial_atencion_edit = new chistorial_atencion_edit();

// Page init
$historial_atencion_edit->Page_Init();

// Page main
$historial_atencion_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$historial_atencion_edit->Page_Render();
?>
...
var CurrentForm = fhistorial_atencionedit = new ew_Form("fhistorial_atencionedit", "edit");

// Validate form
fhistorial_atencionedit.Validate = function() {
...
fhistorial_atencionedit.Form_CustomValidate = 
...
fhistorial_atencionedit.ValidateRequired = true;
<?php } else { ?>
fhistorial_atencionedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$historial_atencion_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $historial_atencion_edit->ShowPageHeader(); ?>
<?php
$historial_atencion_edit->ShowMessage();
?>
<form name="fhistorial_atencionedit" id="fhistorial_atencionedit" class="<?php echo $historial_atencion_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($historial_atencion_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $historial_atencion_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if ($historial_atencion->getCurrentMasterTable() == "atencion_equipos") { ?>
...
<?php if ($historial_atencion->Id_Historial->Visible) { // Id_Historial ?>
	<div id="r_Id_Historial" class="form-group">
		<label id="elh_historial_atencion_Id_Historial" class="col-sm-2 control-label ewLabel"><?php echo $historial_atencion->Id_Historial->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $historial_atencion->Id_Historial->CellAttributes() ?>>
<span id="el_historial_atencion_Id_Historial">
<span<?php echo $historial_atencion->Id_Historial->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $historial_atencion->Id_Historial->EditValue ?></p></span>
</span>
<input type="hidden" data-table="historial_atencion" data-field="x_Id_Historial" name="x_Id_Historial" id="x_Id_Historial" value="<?php echo ew_HtmlEncode($historial_atencion->Id_Historial->CurrentValue) ?>">
<?php echo $historial_atencion->Id_Historial->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($historial_atencion->Detalle->Visible) { // Detalle ?>
	<div id="r_Detalle" class="form-group">
		<label id="elh_historial_atencion_Detalle" for="x_Detalle" class="col-sm-2 control-label ewLabel"><?php echo $historial_atencion->Detalle->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $historial_atencion->Detalle->CellAttributes() ?>>
...
<?php echo $historial_atencion->Detalle->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$historial_atencion_edit->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $historial_atencion_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fhistorial_atencionedit.Init();
</script>
<?php
$historial_atencion_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$historial_atencion_edit->Page_Terminate();
?>
