...
$historial_atencion_delete = NULL; // Initialize page object first

class chistorial_atencion_delete extends chistorial_atencion {
	...
	// Page object name
	var $PageObjName = 'historial_atencion_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("historial_atencionlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in historial_atencion class, historial_atencioninfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("historial_atencionlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($historial_atencion_delete)) $historial_atencion_delete = new chistorial_atencion_delete();

// Page init
$historial_atencion_delete->Page_Init();

// Page main
$historial_atencion_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$historial_atencion_delete->Page_Render();
?>
...
var CurrentForm = fhistorial_atenciondelete = new ew_Form("fhistorial_atenciondelete", "delete");

// Form_CustomValidate event
fhistorial_atenciondelete.Form_CustomValidate = 
...
fhistorial_atenciondelete.ValidateRequired = true;
<?php } else { ?>
fhistorial_atenciondelete.ValidateRequired = false; 
<?php } ?>
...
<?php $historial_atencion_delete->ShowPageHeader(); ?>
<?php
$historial_atencion_delete->ShowMessage();
?>
<form name="fhistorial_atenciondelete" id="fhistorial_atenciondelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($historial_atencion_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $historial_atencion_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="historial_atencion">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($historial_atencion_delete->RecKeys as $key) { ?>
...
<?php if ($historial_atencion->Detalle->Visible) { // Detalle ?>
		<th><span id="elh_historial_atencion_Detalle" class="historial_atencion_Detalle"><?php echo $historial_atencion->Detalle->FldCaption() ?></span></th>
<?php } ?>
<?php if ($historial_atencion->Fecha_Actualizacion->Visible) { // Fecha_Actualizacion ?>
		<th><span id="elh_historial_atencion_Fecha_Actualizacion" class="historial_atencion_Fecha_Actualizacion"><?php echo $historial_atencion->Fecha_Actualizacion->FldCaption() ?></span></th>
<?php } ?>
<?php if ($historial_atencion->Usuario->Visible) { // Usuario ?>
		<th><span id="elh_historial_atencion_Usuario" class="historial_atencion_Usuario"><?php echo $historial_atencion->Usuario->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$historial_atencion_delete->RecCnt = 0;
$i = 0;
while (!$historial_atencion_delete->Recordset->EOF) {
	$historial_atencion_delete->RecCnt++;
	$historial_atencion_delete->RowCnt++;

	// Set row properties
	$historial_atencion->ResetAttrs();
	$historial_atencion->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$historial_atencion_delete->LoadRowValues($historial_atencion_delete->Recordset);

	// Render row
	$historial_atencion_delete->RenderRow();
?>
	<tr<?php echo $historial_atencion->RowAttributes() ?>>
<?php if ($historial_atencion->Detalle->Visible) { // Detalle ?>
		<td<?php echo $historial_atencion->Detalle->CellAttributes() ?>>
<span id="el<?php echo $historial_atencion_delete->RowCnt ?>_historial_atencion_Detalle" class="historial_atencion_Detalle">
...
<?php } ?>
<?php if ($historial_atencion->Fecha_Actualizacion->Visible) { // Fecha_Actualizacion ?>
		<td<?php echo $historial_atencion->Fecha_Actualizacion->CellAttributes() ?>>
<span id="el<?php echo $historial_atencion_delete->RowCnt ?>_historial_atencion_Fecha_Actualizacion" class="historial_atencion_Fecha_Actualizacion">
...
<?php } ?>
<?php if ($historial_atencion->Usuario->Visible) { // Usuario ?>
		<td<?php echo $historial_atencion->Usuario->CellAttributes() ?>>
<span id="el<?php echo $historial_atencion_delete->RowCnt ?>_historial_atencion_Usuario" class="historial_atencion_Usuario">
...
<?php
	$historial_atencion_delete->Recordset->MoveNext();
}
$historial_atencion_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $historial_atencion_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fhistorial_atenciondelete.Init();
</script>
<?php
$historial_atencion_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$historial_atencion_delete->Page_Terminate();
?>
