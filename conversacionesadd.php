...
$conversaciones_add = NULL; // Initialize page object first

class cconversaciones_add extends cconversaciones {
	...
	// Page object name
	var $PageObjName = 'conversaciones_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["Nro_Conversacion"] != "") {
				...
				$this->setKey("Nro_Conversacion", $this->Nro_Conversacion->CurrentValue); // Set up key
			} else {
				$this->setKey("Nro_Conversacion", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "conversacionesview.php")
	...
	function ValidateForm() {
		...
		if (in_array("chat", $DetailTblVar) && $GLOBALS["chat"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("chat", $DetailTblVar) && $GLOBALS["chat"]->DetailAdd) {
				$GLOBALS["chat"]->Nro_Conversacion->setSessionValue($this->Nro_Conversacion->CurrentValue); // Set master key
				if (!isset($GLOBALS["chat_grid"])) $GLOBALS["chat_grid"] = new cchat_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "chat"); // Load user level of detail table
				$AddRow = $GLOBALS["chat_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["chat"]->Nro_Conversacion->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("chat", $DetailTblVar)) {
				...
				if ($GLOBALS["chat_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["chat_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["chat_grid"]->CurrentMode = "add";
					$GLOBALS["chat_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($conversaciones_add)) $conversaciones_add = new cconversaciones_add();

// Page init
$conversaciones_add->Page_Init();

// Page main
$conversaciones_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$conversaciones_add->Page_Render();
?>
...
var CurrentForm = fconversacionesadd = new ew_Form("fconversacionesadd", "add");

// Validate form
fconversacionesadd.Validate = function() {
...
fconversacionesadd.Form_CustomValidate = 
...
fconversacionesadd.ValidateRequired = true;
<?php } else { ?>
fconversacionesadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fconversacionesadd.Lists["x_Usuario_1"] = {"LinkField":"x_Apellido_Nombre","Ajax":true,"AutoFill":false,"DisplayFields":["x_Apellido_Nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"referente_tecnico"};
...
<?php if (!$conversaciones_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $conversaciones_add->ShowPageHeader(); ?>
<?php
$conversaciones_add->ShowMessage();
?>
<form name="fconversacionesadd" id="fconversacionesadd" class="<?php echo $conversaciones_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($conversaciones_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $conversaciones_add->Token ?>">
<?php } ?>
...
<?php
	if (in_array("chat", explode(",", $conversaciones->getCurrentDetailTable())) && $chat->DetailAdd) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $conversaciones_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fconversacionesadd.Init();
</script>
<?php
$conversaciones_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$conversaciones_add->Page_Terminate();
?>
