...
$asistencia_rte_edit = NULL; // Initialize page object first

class casistencia_rte_edit extends casistencia_rte {
	...
	// Page object name
	var $PageObjName = 'asistencia_rte_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->Id_Asistencia->CurrentValue == "") {
			$this->Page_Terminate("asistencia_rtelist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("asistencia_rtelist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "asistencia_rtelist.php")
...
// Create page object
if (!isset($asistencia_rte_edit)) $asistencia_rte_edit = new casistencia_rte_edit();

// Page init
$asistencia_rte_edit->Page_Init();

// Page main
$asistencia_rte_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$asistencia_rte_edit->Page_Render();
?>
...
var CurrentForm = fasistencia_rteedit = new ew_Form("fasistencia_rteedit", "edit");

// Validate form
fasistencia_rteedit.Validate = function() {
...
fasistencia_rteedit.Form_CustomValidate = 
...
fasistencia_rteedit.ValidateRequired = true;
<?php } else { ?>
fasistencia_rteedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fasistencia_rteedit.Lists["x_Mes"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fasistencia_rteedit.Lists["x_Mes"].Options = <?php echo json_encode($asistencia_rte->Mes->Options()) ?>;
fasistencia_rteedit.Lists["x_Ano"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fasistencia_rteedit.Lists["x_Ano"].Options = <?php echo json_encode($asistencia_rte->Ano->Options()) ?>;
...
<?php if (!$asistencia_rte_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $asistencia_rte_edit->ShowPageHeader(); ?>
<?php
$asistencia_rte_edit->ShowMessage();
?>
<form name="fasistencia_rteedit" id="fasistencia_rteedit" class="<?php echo $asistencia_rte_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($asistencia_rte_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $asistencia_rte_edit->Token ?>">
<?php } ?>
...
<?php if ($asistencia_rte->DniRte->Visible) { // DniRte ?>
	<div id="r_DniRte" class="form-group">
		<label id="elh_asistencia_rte_DniRte" for="x_DniRte" class="col-sm-2 control-label ewLabel"><?php echo $asistencia_rte->DniRte->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $asistencia_rte->DniRte->CellAttributes() ?>>
...
<?php echo $asistencia_rte->DniRte->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($asistencia_rte->Mes->Visible) { // Mes ?>
	<div id="r_Mes" class="form-group">
		<label id="elh_asistencia_rte_Mes" for="x_Mes" class="col-sm-2 control-label ewLabel"><?php echo $asistencia_rte->Mes->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $asistencia_rte->Mes->CellAttributes() ?>>
...
<?php echo $asistencia_rte->Mes->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($asistencia_rte->Ano->Visible) { // Ano ?>
	<div id="r_Ano" class="form-group">
		<label id="elh_asistencia_rte_Ano" for="x_Ano" class="col-sm-2 control-label ewLabel"><?php echo $asistencia_rte->Ano->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $asistencia_rte->Ano->CellAttributes() ?>>
...
<?php echo $asistencia_rte->Ano->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<input type="hidden" data-table="asistencia_rte" data-field="x_Id_Asistencia" name="x_Id_Asistencia" id="x_Id_Asistencia" value="<?php echo ew_HtmlEncode($asistencia_rte->Id_Asistencia->CurrentValue) ?>">
<?php if (!$asistencia_rte_edit->IsModal) { ?>
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $asistencia_rte_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fasistencia_rteedit.Init();
</script>
<?php
$asistencia_rte_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$asistencia_rte_edit->Page_Terminate();
?>
