...
class cdepartamento extends cTable {
	...
	function __construct() {
		...
		// Id_Departamento
		$this->Id_Departamento = new cField('departamento', 'departamento', 'x_Id_Departamento', 'Id_Departamento', '`Id_Departamento`', '`Id_Departamento`', 3, -1, FALSE, '`Id_Departamento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// Id_Provincia
		$this->Id_Provincia = new cField('departamento', 'departamento', 'x_Id_Provincia', 'Id_Provincia', '`Id_Provincia`', '`Id_Provincia`', 3, -1, FALSE, '`Id_Provincia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('Id_Departamento', $rs))
				ew_AddFilter($where, ew_QuotedName('Id_Departamento', $this->DBID) . '=' . ew_QuotedValue($rs['Id_Departamento'], $this->Id_Departamento->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->Id_Departamento->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@Id_Departamento@", ew_AdjustSql($this->Id_Departamento->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "Id_Departamento:" . ew_VarToJson($this->Id_Departamento->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->Id_Departamento->CurrentValue)) {
			$sUrl .= "Id_Departamento=" . urlencode($this->Id_Departamento->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["Id_Departamento"]))
				$arKeys[] = ew_StripSlashes($_POST["Id_Departamento"]);
			elseif (isset($_GET["Id_Departamento"]))
				$arKeys[] = ew_StripSlashes($_GET["Id_Departamento"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->Id_Departamento->CurrentValue = $key;
