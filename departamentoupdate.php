...
$departamento_update = NULL; // Initialize page object first

class cdepartamento_update extends cdepartamento {
	...
	// Page object name
	var $PageObjName = 'departamento_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("departamentolist.php"); // No records selected, return to list
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->Id_Departamento->CurrentValue = $sKeyFld;
		return TRUE;
	}
...
// Create page object
if (!isset($departamento_update)) $departamento_update = new cdepartamento_update();

// Page init
$departamento_update->Page_Init();

// Page main
$departamento_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$departamento_update->Page_Render();
?>
...
var CurrentForm = fdepartamentoupdate = new ew_Form("fdepartamentoupdate", "update");

// Validate form
fdepartamentoupdate.Validate = function() {
...
fdepartamentoupdate.Form_CustomValidate = 
...
fdepartamentoupdate.ValidateRequired = true;
<?php } else { ?>
fdepartamentoupdate.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdepartamentoupdate.Lists["x_Id_Provincia"] = {"LinkField":"x_Id_Provincia","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"provincias"};
...
<?php if (!$departamento_update->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $departamento_update->ShowPageHeader(); ?>
<?php
$departamento_update->ShowMessage();
?>
<form name="fdepartamentoupdate" id="fdepartamentoupdate" class="<?php echo $departamento_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($departamento_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $departamento_update->Token ?>">
<?php } ?>
...
<?php } ?>
<?php foreach ($departamento_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_departamentoupdate">
	<div class="checkbox">
...
<input type="checkbox" name="u_Nombre" id="u_Nombre" value="1"<?php echo ($departamento->Nombre->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $departamento->Nombre->FldCaption() ?></label>
...
<input type="checkbox" name="u_Id_Provincia" id="u_Id_Provincia" value="1"<?php echo ($departamento->Id_Provincia->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $departamento->Id_Provincia->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $departamento->Id_Provincia->CellAttributes() ?>>
...
<?php echo $departamento->Id_Provincia->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if (!$departamento_update->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $departamento_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div>
	</div>
<?php } ?>
</div>
...
<script type="text/javascript">
fdepartamentoupdate.Init();
</script>
<?php
$departamento_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$departamento_update->Page_Terminate();
?>
