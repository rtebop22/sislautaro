...
$autoridades_escolares_update = NULL; // Initialize page object first

class cautoridades_escolares_update extends cautoridades_escolares {
	...
	// Page object name
	var $PageObjName = 'autoridades_escolares_update';
	...
	function Page_Init() {
		...
		$this->Id_Autoridad->Visible = !$this->IsAdd() && !$this->IsCopy() && !$this->IsGridAdd();
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("autoridades_escolareslist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->Maill->setDbValue($this->Recordset->fields('Maill'));
					...
					if (!ew_CompareValue($this->Maill->DbValue, $this->Recordset->fields('Maill')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->Id_Autoridad->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->Maill->MultiUpdate = $objForm->GetValue("u_Maill");
	...
	function ValidateForm() {
		...
		if ($this->Maill->MultiUpdate == "1") $lUpdateCnt++;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Maill
			$this->Maill->SetDbValueDef($rsnew, $this->Maill->CurrentValue, NULL, $this->Maill->ReadOnly || $this->Maill->MultiUpdate <> "1");
...
// Create page object
if (!isset($autoridades_escolares_update)) $autoridades_escolares_update = new cautoridades_escolares_update();

// Page init
$autoridades_escolares_update->Page_Init();

// Page main
$autoridades_escolares_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$autoridades_escolares_update->Page_Render();
?>
...
var CurrentForm = fautoridades_escolaresupdate = new ew_Form("fautoridades_escolaresupdate", "update");

// Validate form
fautoridades_escolaresupdate.Validate = function() {
...
fautoridades_escolaresupdate.Form_CustomValidate = 
...
fautoridades_escolaresupdate.ValidateRequired = true;
<?php } else { ?>
fautoridades_escolaresupdate.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fautoridades_escolaresupdate.Lists["x_Id_Cargo"] = {"LinkField":"x_Id_Cargo","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cargo_autoridad"};
fautoridades_escolaresupdate.Lists["x_Id_Turno"] = {"LinkField":"x_Id_Turno","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"turno"};
...
<?php if (!$autoridades_escolares_update->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $autoridades_escolares_update->ShowPageHeader(); ?>
<?php
$autoridades_escolares_update->ShowMessage();
?>
<form name="fautoridades_escolaresupdate" id="fautoridades_escolaresupdate" class="<?php echo $autoridades_escolares_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($autoridades_escolares_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $autoridades_escolares_update->Token ?>">
<?php } ?>
...
<?php } ?>
<?php foreach ($autoridades_escolares_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_autoridades_escolaresupdate">
	<div class="checkbox">
...
<input type="checkbox" name="u_Apellido_Nombre" id="u_Apellido_Nombre" value="1"<?php echo ($autoridades_escolares->Apellido_Nombre->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $autoridades_escolares->Apellido_Nombre->FldCaption() ?></label>
...
<input type="checkbox" name="u_Cuil" id="u_Cuil" value="1"<?php echo ($autoridades_escolares->Cuil->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $autoridades_escolares->Cuil->FldCaption() ?></label>
...
<input type="checkbox" name="u_Id_Cargo" id="u_Id_Cargo" value="1"<?php echo ($autoridades_escolares->Id_Cargo->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $autoridades_escolares->Id_Cargo->FldCaption() ?></label>
...
<input type="checkbox" name="u_Id_Turno" id="u_Id_Turno" value="1"<?php echo ($autoridades_escolares->Id_Turno->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $autoridades_escolares->Id_Turno->FldCaption() ?></label>
...
<input type="checkbox" name="u_Telefono" id="u_Telefono" value="1"<?php echo ($autoridades_escolares->Telefono->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $autoridades_escolares->Telefono->FldCaption() ?></label>
...
<input type="checkbox" name="u_Celular" id="u_Celular" value="1"<?php echo ($autoridades_escolares->Celular->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $autoridades_escolares->Celular->FldCaption() ?></label>
...
<?php if ($autoridades_escolares->Maill->Visible) { // Maill ?>
	<div id="r_Maill" class="form-group">
		<label for="x_Maill" class="col-sm-2 control-label">
<input type="checkbox" name="u_Maill" id="u_Maill" value="1"<?php echo ($autoridades_escolares->Maill->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $autoridades_escolares->Maill->FldCaption() ?></label>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $autoridades_escolares_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div>
	</div>
<?php } ?>
</div>
...
<script type="text/javascript">
fautoridades_escolaresupdate.Init();
</script>
<?php
$autoridades_escolares_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$autoridades_escolares_update->Page_Terminate();
?>
