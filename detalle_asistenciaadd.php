...
$detalle_asistencia_add = NULL; // Initialize page object first

class cdetalle_asistencia_add extends cdetalle_asistencia {
	...
	// Page object name
	var $PageObjName = 'detalle_asistencia_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["Dia"] != "") {
				...
				$this->setKey("Dia", $this->Dia->CurrentValue); // Set up key
			} else {
				$this->setKey("Dia", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "detalle_asistenciaview.php")
...
// Create page object
if (!isset($detalle_asistencia_add)) $detalle_asistencia_add = new cdetalle_asistencia_add();

// Page init
$detalle_asistencia_add->Page_Init();

// Page main
$detalle_asistencia_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$detalle_asistencia_add->Page_Render();
?>
...
var CurrentForm = fdetalle_asistenciaadd = new ew_Form("fdetalle_asistenciaadd", "add");

// Validate form
fdetalle_asistenciaadd.Validate = function() {
...
fdetalle_asistenciaadd.Form_CustomValidate = 
...
fdetalle_asistenciaadd.ValidateRequired = true;
<?php } else { ?>
fdetalle_asistenciaadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$detalle_asistencia_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $detalle_asistencia_add->ShowPageHeader(); ?>
<?php
$detalle_asistencia_add->ShowMessage();
?>
<form name="fdetalle_asistenciaadd" id="fdetalle_asistenciaadd" class="<?php echo $detalle_asistencia_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($detalle_asistencia_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $detalle_asistencia_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $detalle_asistencia_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fdetalle_asistenciaadd.Init();
</script>
<?php
$detalle_asistencia_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$detalle_asistencia_add->Page_Terminate();
?>
