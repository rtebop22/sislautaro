...
class catencion_equipos_add extends catencion_equipos {
	...
	function ValidateForm() {
		...
		if (in_array("historial_atencion", $DetailTblVar) && $GLOBALS["historial_atencion"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			...
			if (in_array("detalle_atencion", $DetailTblVar) && $GLOBALS["detalle_atencion"]->DetailAdd) {
				...
				$GLOBALS["detalle_atencion"]->NroSerie->setSessionValue($this->NroSerie->CurrentValue); // Set master key
				if (!isset($GLOBALS["detalle_atencion_grid"])) $GLOBALS["detalle_atencion_grid"] = new cdetalle_atencion_grid(); // Get detail page object
				...
				if (!$AddRow)
					$GLOBALS["detalle_atencion"]->NroSerie->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("atencion_para_st", $DetailTblVar) && $GLOBALS["atencion_para_st"]->DetailAdd) {
				$GLOBALS["atencion_para_st"]->Id_Atencion->setSessionValue($this->Id_Atencion->CurrentValue); // Set master key
				$GLOBALS["atencion_para_st"]->NroSerie->setSessionValue($this->NroSerie->CurrentValue); // Set master key
				if (!isset($GLOBALS["atencion_para_st_grid"])) $GLOBALS["atencion_para_st_grid"] = new catencion_para_st_grid(); // Get detail page object
				...
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["atencion_para_st"]->NroSerie->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("historial_atencion", $DetailTblVar) && $GLOBALS["historial_atencion"]->DetailAdd) {
				$GLOBALS["historial_atencion"]->Id_Atencion->setSessionValue($this->Id_Atencion->CurrentValue); // Set master key
				$GLOBALS["historial_atencion"]->NroSerie->setSessionValue($this->NroSerie->CurrentValue); // Set master key
				if (!isset($GLOBALS["historial_atencion_grid"])) $GLOBALS["historial_atencion_grid"] = new chistorial_atencion_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "historial_atencion"); // Load user level of detail table
				$AddRow = $GLOBALS["historial_atencion_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["historial_atencion"]->NroSerie->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("historial_atencion", $DetailTblVar)) {
				...
				if ($GLOBALS["historial_atencion_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["historial_atencion_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["historial_atencion_grid"]->CurrentMode = "add";
					$GLOBALS["historial_atencion_grid"]->CurrentAction = "gridadd";
...
<?php } ?>
<?php
	if (in_array("historial_atencion", explode(",", $atencion_equipos->getCurrentDetailTable())) && $historial_atencion->DetailAdd) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "historial_atencion") {
			$FirstActiveDetailTable = "historial_atencion";
		}
?>
	<div class="panel panel-default<?php echo $atencion_equipos_add->DetailPages->PageStyle("historial_atencion") ?>">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a class="panel-toggle" data-toggle="collapse" data-parent="#atencion_equipos_add_details" href="#tab_historial_atencion"><?php echo $Language->TablePhrase("historial_atencion", "TblCaption") ?></a>
			</h4>
		</div>
		<div class="panel-collapse collapse<?php echo $atencion_equipos_add->DetailPages->PageStyle("historial_atencion") ?>" id="tab_historial_atencion">
			<div class="panel-body">
