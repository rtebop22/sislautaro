...
class cprestamo_equipo_update extends cprestamo_equipo {
	...
	function LoadMultiUpdateValues() {
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					$this->Apellidos_Nombres_Beneficiario->setDbValue($this->Recordset->fields('Apellidos_Nombres_Beneficiario'));
					...
					if (!ew_CompareValue($this->Apellidos_Nombres_Beneficiario->DbValue, $this->Recordset->fields('Apellidos_Nombres_Beneficiario')))
	...
	function LoadFormValues() {
		...
		$this->Apellidos_Nombres_Beneficiario->MultiUpdate = $objForm->GetValue("u_Apellidos_Nombres_Beneficiario");
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($this->Apellidos_Nombres_Beneficiario->MultiUpdate == "1") $lUpdateCnt++;
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($this->Apellidos_Nombres_Beneficiario->MultiUpdate <> "" && !$this->Apellidos_Nombres_Beneficiario->FldIsDetailKey && !is_null($this->Apellidos_Nombres_Beneficiario->FormValue) && $this->Apellidos_Nombres_Beneficiario->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Apellidos_Nombres_Beneficiario
			$this->Apellidos_Nombres_Beneficiario->SetDbValueDef($rsnew, $this->Apellidos_Nombres_Beneficiario->CurrentValue, NULL, $this->Apellidos_Nombres_Beneficiario->ReadOnly || $this->Apellidos_Nombres_Beneficiario->MultiUpdate <> "1");
			...
			// Fecha_Prestamo
			$this->Fecha_Prestamo->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->Fecha_Prestamo->CurrentValue, 7), NULL, $this->Fecha_Prestamo->ReadOnly || $this->Fecha_Prestamo->MultiUpdate <> "1");
...
fprestamo_equipoupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_Apellidos_Nombres_Beneficiario");
...
fprestamo_equipoupdate.Lists["x_Apellidos_Nombres_Beneficiario"] = {"LinkField":"x_Apellidos_Nombres","Ajax":true,"AutoFill":true,"DisplayFields":["x_Apellidos_Nombres","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"lista_espera_prestamo"};
...
fprestamo_equipoupdate.Lists["x_Devuelve_Cargador"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fprestamo_equipoupdate.Lists["x_Devuelve_Cargador"].Options = <?php echo json_encode($prestamo_equipo->Devuelve_Cargador->Options()) ?>;
...
<?php if ($prestamo_equipo->Apellidos_Nombres_Beneficiario->Visible) { // Apellidos_Nombres_Beneficiario ?>
	<div id="r_Apellidos_Nombres_Beneficiario" class="form-group">
		<label for="x_Apellidos_Nombres_Beneficiario" class="col-sm-2 control-label">
<input type="checkbox" name="u_Apellidos_Nombres_Beneficiario" id="u_Apellidos_Nombres_Beneficiario" value="1"<?php echo ($prestamo_equipo->Apellidos_Nombres_Beneficiario->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $prestamo_equipo->Apellidos_Nombres_Beneficiario->FldCaption() ?></label>
...
<?php if ($prestamo_equipo->Devuelve_Cargador->Visible) { // Devuelve_Cargador ?>
	<div id="r_Devuelve_Cargador" class="form-group">
		<label class="col-sm-2 control-label">
<input type="checkbox" name="u_Devuelve_Cargador" id="u_Devuelve_Cargador" value="1"<?php echo ($prestamo_equipo->Devuelve_Cargador->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $prestamo_equipo->Devuelve_Cargador->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $prestamo_equipo->Devuelve_Cargador->CellAttributes() ?>>
...
<?php echo $prestamo_equipo->Devuelve_Cargador->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if (!$prestamo_equipo_update->IsModal) { ?>
