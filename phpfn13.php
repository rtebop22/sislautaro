...
function CurrentProjectID() {
	...
	return "{9FD9BA28-0339-4B41-9A45-0CAE935EFE3A}";
}
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($validateUser) {
						...
						$this->setSessionUserID($rs->fields('Nombre')); // Load User ID
						if (is_null($rs->fields('Nivel_Usuario'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('Nivel_Usuario'))); // Load User Level
						}
						$this->SetUpUserLevel();
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $UserTableConn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('Nombre'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
...
// Show barcode
function ew_ShowBarCode($barcode, $encode, $height = "0", $format = "png", $font = "", $scale = "", $color = "#000000", $bgcolor = "#FFFFFF") {
	global $gsExport, $gsCustomExport;
	if (ew_EmptyStr($barcode))
		return "";
	if ($gsExport == "" || $gsExport == "print" && ($gsCustomExport == "" || $gsCustomExport == "print")) {
		$url = "data=" . urlencode($barcode) . "&amp;encode=" . urlencode($encode);
		if ($format <> "") $url .= "&amp;format=" . urlencode($format);
		if ($font == "") $font = EW_TMP_IMAGE_FONT;
		if ($font <> "") $url .= "&amp;font=" . urlencode($font);
		if ($height <> "0") $url .= "&amp;height=" . urlencode($height);
		if ($scale <> "") $url .= "&amp;scale=" . urlencode($scale);
		if ($color <> "") $url .= "&amp;color=" . urlencode($color);
		if ($bgcolor <> "") $url .= "&amp;bgcolor=" . urlencode($bgcolor);
		return "<img src=\"phpinc/ewbarcode.php?" . $url . "\" alt=\"\">";
	} elseif ($gsExport == "print" && ($gsCustomExport == "pdf" || $gsCustomExport == "email") || $gsExport == "excel" && defined('EW_USE_PHPEXCEL') || $gsExport == "word" && defined('EW_USE_PHPWORD')) {
		$ar = array("barcode" => $barcode, "encode" => $encode, "height" => $height, "format" => $format, "font" => $font, "scale" => $scale, "color" => $color, "bgcolor" => $bgcolor);
		return ew_BarCodeImageTag($ar, $gsCustomExport);
	} else {
		return $barcode;
	}
}
...
// Get barcode <img> tag
function ew_BarCodeImageTag($ar, $export = "") {
	global $gTmpImages;
	if ($export == "") {
		if (@$_GET["export"] <> "")
			$export = $_GET["export"];
	...
	if ($barcode <> "") {
		...
		if ($encode == "DATAMATRIX" || $encode == "QRCODE") {
			include_once "phpinc/tcpdf_barcodes_2d.php";
			...
			$data = $dm->getBarcodePngData($h, $h, $c);
			file_put_contents($oldfn . "." . $format, $data);
		} else {
			include_once "phpinc/barcode.inc.php";
			$bar = new BARCODE();
			...
			$bar->genBarCode($barcode, $format, $oldfn);
		}
		$tmpimage = basename($fn);
		$gTmpImages[] = $tmpimage;
		if ($export == "excel" && defined('EW_USE_PHPEXCEL') || $export == "word" && defined('EW_USE_PHPWORD'))
			return ew_TmpImageLnk($tmpimage, $export);
		else
			return "<img src=\"" . ew_TmpImageLnk($tmpimage, $export) . "\">";
	} else {
		return "";
	}
}
