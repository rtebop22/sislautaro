...
$asistencia_rte_add = NULL; // Initialize page object first

class casistencia_rte_add extends casistencia_rte {
	...
	// Page object name
	var $PageObjName = 'asistencia_rte_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["Id_Asistencia"] != "") {
				...
				$this->setKey("Id_Asistencia", $this->Id_Asistencia->CurrentValue); // Set up key
			} else {
				$this->setKey("Id_Asistencia", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "asistencia_rteview.php")
...
// Create page object
if (!isset($asistencia_rte_add)) $asistencia_rte_add = new casistencia_rte_add();

// Page init
$asistencia_rte_add->Page_Init();

// Page main
$asistencia_rte_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$asistencia_rte_add->Page_Render();
?>
...
var CurrentForm = fasistencia_rteadd = new ew_Form("fasistencia_rteadd", "add");

// Validate form
fasistencia_rteadd.Validate = function() {
...
fasistencia_rteadd.Form_CustomValidate = 
...
fasistencia_rteadd.ValidateRequired = true;
<?php } else { ?>
fasistencia_rteadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fasistencia_rteadd.Lists["x_Mes"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fasistencia_rteadd.Lists["x_Mes"].Options = <?php echo json_encode($asistencia_rte->Mes->Options()) ?>;
fasistencia_rteadd.Lists["x_Ano"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fasistencia_rteadd.Lists["x_Ano"].Options = <?php echo json_encode($asistencia_rte->Ano->Options()) ?>;
...
<?php if (!$asistencia_rte_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $asistencia_rte_add->ShowPageHeader(); ?>
<?php
$asistencia_rte_add->ShowMessage();
?>
<form name="fasistencia_rteadd" id="fasistencia_rteadd" class="<?php echo $asistencia_rte_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($asistencia_rte_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $asistencia_rte_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $asistencia_rte_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fasistencia_rteadd.Init();
</script>
<?php
$asistencia_rte_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$asistencia_rte_add->Page_Terminate();
?>
