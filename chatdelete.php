...
$chat_delete = NULL; // Initialize page object first

class cchat_delete extends cchat {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("chatlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in chat class, chatinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("chatlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($chat_delete)) $chat_delete = new cchat_delete();

// Page init
$chat_delete->Page_Init();

// Page main
$chat_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$chat_delete->Page_Render();
?>
...
var CurrentForm = fchatdelete = new ew_Form("fchatdelete", "delete");

// Form_CustomValidate event
fchatdelete.Form_CustomValidate = 
...
fchatdelete.ValidateRequired = true;
<?php } else { ?>
fchatdelete.ValidateRequired = false; 
<?php } ?>
...
<?php $chat_delete->ShowPageHeader(); ?>
<?php
$chat_delete->ShowMessage();
?>
<form name="fchatdelete" id="fchatdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($chat_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $chat_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="chat">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($chat_delete->RecKeys as $key) { ?>
...
<?php if ($chat->Texto_Chat->Visible) { // Texto_Chat ?>
		<th><span id="elh_chat_Texto_Chat" class="chat_Texto_Chat"><?php echo $chat->Texto_Chat->FldCaption() ?></span></th>
<?php } ?>
<?php if ($chat->Hora->Visible) { // Hora ?>
		<th><span id="elh_chat_Hora" class="chat_Hora"><?php echo $chat->Hora->FldCaption() ?></span></th>
<?php } ?>
<?php if ($chat->Usuario->Visible) { // Usuario ?>
		<th><span id="elh_chat_Usuario" class="chat_Usuario"><?php echo $chat->Usuario->FldCaption() ?></span></th>
<?php } ?>
<?php if ($chat->Estado->Visible) { // Estado ?>
		<th><span id="elh_chat_Estado" class="chat_Estado"><?php echo $chat->Estado->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$chat_delete->RecCnt = 0;
$i = 0;
while (!$chat_delete->Recordset->EOF) {
	$chat_delete->RecCnt++;
	$chat_delete->RowCnt++;

	// Set row properties
	$chat->ResetAttrs();
	$chat->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$chat_delete->LoadRowValues($chat_delete->Recordset);

	// Render row
	$chat_delete->RenderRow();
?>
	<tr<?php echo $chat->RowAttributes() ?>>
<?php if ($chat->Texto_Chat->Visible) { // Texto_Chat ?>
		<td<?php echo $chat->Texto_Chat->CellAttributes() ?>>
<span id="el<?php echo $chat_delete->RowCnt ?>_chat_Texto_Chat" class="chat_Texto_Chat">
...
<?php } ?>
<?php if ($chat->Hora->Visible) { // Hora ?>
		<td<?php echo $chat->Hora->CellAttributes() ?>>
<span id="el<?php echo $chat_delete->RowCnt ?>_chat_Hora" class="chat_Hora">
...
<?php } ?>
<?php if ($chat->Usuario->Visible) { // Usuario ?>
		<td<?php echo $chat->Usuario->CellAttributes() ?>>
<span id="el<?php echo $chat_delete->RowCnt ?>_chat_Usuario" class="chat_Usuario">
...
<?php } ?>
<?php if ($chat->Estado->Visible) { // Estado ?>
		<td<?php echo $chat->Estado->CellAttributes() ?>>
<span id="el<?php echo $chat_delete->RowCnt ?>_chat_Estado" class="chat_Estado">
...
<?php
	$chat_delete->Recordset->MoveNext();
}
$chat_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $chat_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fchatdelete.Init();
