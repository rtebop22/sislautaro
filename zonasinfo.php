...
//
// Table class for zonas
//
class czonas extends cTable {
	var $Id_Zona;
	var $Descripcion;
	var $Id_Departamento;
	...
	function __construct() {
		...
		// Id_Zona
		$this->Id_Zona = new cField('zonas', 'zonas', 'x_Id_Zona', 'Id_Zona', '`Id_Zona`', '`Id_Zona`', 3, -1, FALSE, '`Id_Zona`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// Descripcion
		$this->Descripcion = new cField('zonas', 'zonas', 'x_Descripcion', 'Descripcion', '`Descripcion`', '`Descripcion`', 200, -1, FALSE, '`Descripcion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Descripcion->Sortable = TRUE; // Allow sort
		$this->fields['Descripcion'] = &$this->Descripcion;

		// Id_Departamento
		$this->Id_Departamento = new cField('zonas', 'zonas', 'x_Id_Departamento', 'Id_Departamento', '`Id_Departamento`', '`Id_Departamento`', 200, -1, FALSE, '`Id_Departamento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Id_Departamento->Sortable = TRUE; // Allow sort
		$this->fields['Id_Departamento'] = &$this->Id_Departamento;

		// Fecha_Actualizacion
		$this->Fecha_Actualizacion = new cField('zonas', 'zonas', 'x_Fecha_Actualizacion', 'Fecha_Actualizacion', '`Fecha_Actualizacion`', 'DATE_FORMAT(`Fecha_Actualizacion`, \'\')', 133, 0, FALSE, '`Fecha_Actualizacion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Usuario
		$this->Usuario = new cField('zonas', 'zonas', 'x_Usuario', 'Usuario', '`Usuario`', '`Usuario`', 201, -1, FALSE, '`Usuario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`zonas`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('Id_Zona', $rs))
				ew_AddFilter($where, ew_QuotedName('Id_Zona', $this->DBID) . '=' . ew_QuotedValue($rs['Id_Zona'], $this->Id_Zona->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`Id_Zona` = @Id_Zona@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->Id_Zona->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@Id_Zona@", ew_AdjustSql($this->Id_Zona->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("zonasview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("zonasview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "zonasadd.php?" . $this->UrlParm($parm);
		else
			$url = "zonasadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("zonasedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("zonasadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("zonasdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "Id_Zona:" . ew_VarToJson($this->Id_Zona->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->Id_Zona->CurrentValue)) {
			$sUrl .= "Id_Zona=" . urlencode($this->Id_Zona->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["Id_Zona"]))
				$arKeys[] = ew_StripSlashes($_POST["Id_Zona"]);
			elseif (isset($_GET["Id_Zona"]))
				$arKeys[] = ew_StripSlashes($_GET["Id_Zona"]);
			else
