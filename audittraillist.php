...
class caudittrail_list extends caudittrail {
	...
	function RenderListOptions() {
		...
		if ($Security->CanView()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewView\" title=\"" . $viewcaption . "\" data-table=\"audittrail\" data-caption=\"" . $viewcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,url:'" . ew_HtmlEncode($this->ViewUrl) . "'});\">" . $Language->Phrase("ViewLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
		...
		if ($Security->CanEdit()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewEdit\" title=\"" . $editcaption . "\" data-table=\"audittrail\" data-caption=\"" . $editcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,url:'" . ew_HtmlEncode($this->EditUrl) . "',caption:'" . $editcaption . "'});\">" . $Language->Phrase("EditLink") . "</a>";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.faudittraillist,url:'" . $this->MultiDeleteUrl . "',msg:ewLanguage.Phrase('DeleteConfirmMsg')});return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->CanDelete());

		// Add multi update
		$item = &$option->Add("multiupdate");
		$item->Body = "<a class=\"ewAction ewMultiUpdate\" title=\"" . ew_HtmlTitle($Language->Phrase("UpdateSelectedLink")) . "\" data-table=\"audittrail\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("UpdateSelectedLink")) . "\" href=\"\" onclick=\"ew_ModalDialogShow({lnk:this,f:document.faudittraillist,url:'" . $this->MultiUpdateUrl . "',caption:'" . $Language->Phrase("UpdateBtn") . "'});return false;\">" . $Language->Phrase("UpdateSelectedLink") . "</a>";
		$item->Visible = ($Security->CanEdit());
...
<?php if ($audittrail_list->TotalRecs > 0 || $audittrail->CurrentAction == "add" || $audittrail->CurrentAction == "copy") { ?>
...
<tbody>
<?php
	if ($audittrail->CurrentAction == "add" || $audittrail->CurrentAction == "copy") {
		$audittrail_list->RowIndex = 0;
		$audittrail_list->KeyCount = $audittrail_list->RowIndex;
		if ($audittrail->CurrentAction == "add")
			$audittrail_list->LoadDefaultValues();
		if ($audittrail->EventCancelled) // Insert failed
			$audittrail_list->RestoreFormValues(); // Restore form values

		// Set row properties
		$audittrail->ResetAttrs();
		$audittrail->RowAttrs = array_merge($audittrail->RowAttrs, array('data-rowindex'=>0, 'id'=>'r0_audittrail', 'data-rowtype'=>EW_ROWTYPE_ADD));
...
$audittrail->ResetAttrs();
$audittrail_list->RenderRow();
$audittrail_list->EditRowCnt = 0;
if ($audittrail->CurrentAction == "edit")
	$audittrail_list->RowIndex = 1;
if ($audittrail->CurrentAction == "gridadd")
	$audittrail_list->RowIndex = 0;
...
while ($audittrail_list->RecCnt < $audittrail_list->StopRec) {
	...
	if (intval($audittrail_list->RecCnt) >= intval($audittrail_list->StartRec)) {
		...
		if ($audittrail->CurrentAction == "edit") {
			if ($audittrail_list->CheckInlineEditKey() && $audittrail_list->EditRowCnt == 0) { // Inline edit
		...
		if ($audittrail->CurrentAction == "edit" && $audittrail->RowType == EW_ROWTYPE_EDIT && $audittrail->EventCancelled) { // Update failed
			$objForm->Index = 1;
			$audittrail_list->RestoreFormValues(); // Restore form values
		}
...
<?php } ?>
<?php if ($audittrail->CurrentAction == "add" || $audittrail->CurrentAction == "copy") { ?>
...
<?php } ?>
<?php if ($audittrail->CurrentAction == "edit") { ?>
<input type="hidden" name="<?php echo $audittrail_list->FormKeyCountName ?>" id="<?php echo $audittrail_list->FormKeyCountName ?>" value="<?php echo $audittrail_list->KeyCount ?>">
<?php } ?>
