...
class cautoridades_escolares_list extends cautoridades_escolares {
	...
	function ClearInlineMode() {
		$this->setKey("Id_Autoridad", ""); // Clear inline edit key
	...
	function InlineEditMode() {
		...
		if ($bInlineEdit) {
			if ($this->LoadRow()) {
				$this->setKey("Id_Autoridad", $this->Id_Autoridad->CurrentValue); // Set up inline edit key
	...
	function CheckInlineEditKey() {
		...
		if (strval($this->getKey("Id_Autoridad")) <> strval($this->Id_Autoridad->CurrentValue))
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentAction == "edit" && $this->RowType == EW_ROWTYPE_EDIT) { // Inline-Edit
			...
			$oListOpt->Body .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . ew_HtmlEncode($this->Id_Autoridad->CurrentValue) . "\">";
			return;
		}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiUpdate\" title=\"" . ew_HtmlTitle($Language->Phrase("UpdateSelectedLink")) . "\" data-table=\"autoridades_escolares\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("UpdateSelectedLink")) . "\" href=\"\" onclick=\"ew_ModalDialogShow({lnk:this,f:document.fautoridades_escolareslist,url:'" . $this->MultiUpdateUrl . "',caption:'" . $Language->Phrase("UpdateBtn") . "'});return false;\">" . $Language->Phrase("UpdateSelectedLink") . "</a>";
		$item->Visible = ($Security->CanEdit());
...
if ($autoridades_escolares->CurrentAction == "gridadd") {
	...
	if ($autoridades_escolares->CurrentAction == "" && $autoridades_escolares_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$autoridades_escolares_list->setWarningMessage(ew_DeniedMsg());
	...
	if ($autoridades_escolares_list->AuditTrailOnSearch && $autoridades_escolares_list->Command == "search" && !$autoridades_escolares_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $autoridades_escolares_list->getSessionWhere();
		$autoridades_escolares_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
}
...
<?php if ($autoridades_escolares_list->TotalRecs > 0 || $autoridades_escolares->CurrentAction == "add" || $autoridades_escolares->CurrentAction == "copy") { ?>
...
<tbody>
<?php
	if ($autoridades_escolares->CurrentAction == "add" || $autoridades_escolares->CurrentAction == "copy") {
		$autoridades_escolares_list->RowIndex = 0;
		$autoridades_escolares_list->KeyCount = $autoridades_escolares_list->RowIndex;
		if ($autoridades_escolares->CurrentAction == "add")
			$autoridades_escolares_list->LoadDefaultValues();
		if ($autoridades_escolares->EventCancelled) // Insert failed
			$autoridades_escolares_list->RestoreFormValues(); // Restore form values

		// Set row properties
		$autoridades_escolares->ResetAttrs();
		$autoridades_escolares->RowAttrs = array_merge($autoridades_escolares->RowAttrs, array('data-rowindex'=>0, 'id'=>'r0_autoridades_escolares', 'data-rowtype'=>EW_ROWTYPE_ADD));
...
$autoridades_escolares->ResetAttrs();
$autoridades_escolares_list->RenderRow();
$autoridades_escolares_list->EditRowCnt = 0;
if ($autoridades_escolares->CurrentAction == "edit")
	$autoridades_escolares_list->RowIndex = 1;
if ($autoridades_escolares->CurrentAction == "gridadd")
	$autoridades_escolares_list->RowIndex = 0;
...
while ($autoridades_escolares_list->RecCnt < $autoridades_escolares_list->StopRec) {
	...
	if (intval($autoridades_escolares_list->RecCnt) >= intval($autoridades_escolares_list->StartRec)) {
		...
		if ($autoridades_escolares->CurrentAction == "edit") {
			if ($autoridades_escolares_list->CheckInlineEditKey() && $autoridades_escolares_list->EditRowCnt == 0) { // Inline edit
		...
		if ($autoridades_escolares->CurrentAction == "edit" && $autoridades_escolares->RowType == EW_ROWTYPE_EDIT && $autoridades_escolares->EventCancelled) { // Update failed
			$objForm->Index = 1;
			$autoridades_escolares_list->RestoreFormValues(); // Restore form values
		}
...
<?php } ?>
<?php if ($autoridades_escolares->CurrentAction == "add" || $autoridades_escolares->CurrentAction == "copy") { ?>
...
<?php } ?>
<?php if ($autoridades_escolares->CurrentAction == "edit") { ?>
<input type="hidden" name="<?php echo $autoridades_escolares_list->FormKeyCountName ?>" id="<?php echo $autoridades_escolares_list->FormKeyCountName ?>" value="<?php echo $autoridades_escolares_list->KeyCount ?>">
<?php } ?>
