...
$chat_view = NULL; // Initialize page object first

class cchat_view extends cchat {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["Orden"] <> "") {
			$this->RecKey["Orden"] = $_GET["Orden"];
			$KeyUrl .= "&amp;Orden=" . urlencode($this->RecKey["Orden"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["Orden"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["Orden"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["Orden"] <> "") {
				...
				$this->RecKey["Orden"] = $this->Orden->QueryStringValue;
			} elseif (@$_POST["Orden"] <> "") {
				$this->Orden->setFormValue($_POST["Orden"]);
				$this->RecKey["Orden"] = $this->Orden->FormValue;
			} else {
				$sReturnUrl = "chatlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "chatlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "chatlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_chat\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_chat',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fchatview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($chat_view)) $chat_view = new cchat_view();
...
var CurrentForm = fchatview = new ew_Form("fchatview", "view");

// Form_CustomValidate event
fchatview.Form_CustomValidate = 
...
fchatview.ValidateRequired = true;
<?php } else { ?>
fchatview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($chat->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$chat_view->IsModal) { ?>
<?php if ($chat->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $chat_view->ExportOptions->Render("body") ?>
<?php
	foreach ($chat_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$chat_view->IsModal) { ?>
<?php if ($chat->Export == "") { ?>
...
<?php } ?>
<?php $chat_view->ShowPageHeader(); ?>
<?php
$chat_view->ShowMessage();
?>
<form name="fchatview" id="fchatview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($chat_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $chat_view->Token ?>">
<?php } ?>
...
<?php if ($chat->Orden->Visible) { // Orden ?>
	<tr id="r_Orden">
		<td><span id="elh_chat_Orden"><?php echo $chat->Orden->FldCaption() ?></span></td>
		<td data-name="Orden"<?php echo $chat->Orden->CellAttributes() ?>>
<span id="el_chat_Orden">
<span<?php echo $chat->Orden->ViewAttributes() ?>>
<?php echo $chat->Orden->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($chat->Texto_Chat->Visible) { // Texto_Chat ?>
	<tr id="r_Texto_Chat">
		<td><span id="elh_chat_Texto_Chat"><?php echo $chat->Texto_Chat->FldCaption() ?></span></td>
...
<?php echo $chat->Texto_Chat->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($chat->Usuario->Visible) { // Usuario ?>
	<tr id="r_Usuario">
		<td><span id="elh_chat_Usuario"><?php echo $chat->Usuario->FldCaption() ?></span></td>
...
<?php echo $chat->Usuario->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($chat->Nro_Conversacion->Visible) { // Nro_Conversacion ?>
	<tr id="r_Nro_Conversacion">
		<td><span id="elh_chat_Nro_Conversacion"><?php echo $chat->Nro_Conversacion->FldCaption() ?></span></td>
		<td data-name="Nro_Conversacion"<?php echo $chat->Nro_Conversacion->CellAttributes() ?>>
<span id="el_chat_Nro_Conversacion">
<span<?php echo $chat->Nro_Conversacion->ViewAttributes() ?>>
<?php echo $chat->Nro_Conversacion->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
