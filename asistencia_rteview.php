...
$asistencia_rte_view = NULL; // Initialize page object first

class casistencia_rte_view extends casistencia_rte {
	...
	// Page object name
	var $PageObjName = 'asistencia_rte_view';
	...
	function __construct() {
		...
		if (@$_GET["Id_Asistencia"] <> "") {
			$this->RecKey["Id_Asistencia"] = $_GET["Id_Asistencia"];
			$KeyUrl .= "&amp;Id_Asistencia=" . urlencode($this->RecKey["Id_Asistencia"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["Id_Asistencia"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["Id_Asistencia"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["Id_Asistencia"] <> "") {
				...
				$this->RecKey["Id_Asistencia"] = $this->Id_Asistencia->QueryStringValue;
			} elseif (@$_POST["Id_Asistencia"] <> "") {
				$this->Id_Asistencia->setFormValue($_POST["Id_Asistencia"]);
				$this->RecKey["Id_Asistencia"] = $this->Id_Asistencia->FormValue;
			} else {
				$sReturnUrl = "asistencia_rtelist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "asistencia_rtelist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "asistencia_rtelist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($asistencia_rte_view)) $asistencia_rte_view = new casistencia_rte_view();

// Page init
$asistencia_rte_view->Page_Init();

// Page main
$asistencia_rte_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$asistencia_rte_view->Page_Render();
?>
...
var CurrentForm = fasistencia_rteview = new ew_Form("fasistencia_rteview", "view");

// Form_CustomValidate event
fasistencia_rteview.Form_CustomValidate = 
...
fasistencia_rteview.ValidateRequired = true;
<?php } else { ?>
fasistencia_rteview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fasistencia_rteview.Lists["x_Mes"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fasistencia_rteview.Lists["x_Mes"].Options = <?php echo json_encode($asistencia_rte->Mes->Options()) ?>;
fasistencia_rteview.Lists["x_Ano"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fasistencia_rteview.Lists["x_Ano"].Options = <?php echo json_encode($asistencia_rte->Ano->Options()) ?>;
...
<?php if ($asistencia_rte->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$asistencia_rte_view->IsModal) { ?>
<?php if ($asistencia_rte->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $asistencia_rte_view->ExportOptions->Render("body") ?>
<?php
	foreach ($asistencia_rte_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$asistencia_rte_view->IsModal) { ?>
<?php if ($asistencia_rte->Export == "") { ?>
...
<?php } ?>
<?php $asistencia_rte_view->ShowPageHeader(); ?>
<?php
$asistencia_rte_view->ShowMessage();
?>
<form name="fasistencia_rteview" id="fasistencia_rteview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($asistencia_rte_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $asistencia_rte_view->Token ?>">
<?php } ?>
...
<?php if ($asistencia_rte->DniRte->Visible) { // DniRte ?>
	<tr id="r_DniRte">
		<td><span id="elh_asistencia_rte_DniRte"><?php echo $asistencia_rte->DniRte->FldCaption() ?></span></td>
...
<?php echo $asistencia_rte->DniRte->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($asistencia_rte->Mes->Visible) { // Mes ?>
	<tr id="r_Mes">
		<td><span id="elh_asistencia_rte_Mes"><?php echo $asistencia_rte->Mes->FldCaption() ?></span></td>
...
<?php echo $asistencia_rte->Mes->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($asistencia_rte->Ano->Visible) { // Ano ?>
	<tr id="r_Ano">
		<td><span id="elh_asistencia_rte_Ano"><?php echo $asistencia_rte->Ano->FldCaption() ?></span></td>
...
<?php echo $asistencia_rte->Ano->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($asistencia_rte->Export == "") { ?>
<script type="text/javascript">
fasistencia_rteview.Init();
</script>
<?php } ?>
<?php
$asistencia_rte_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($asistencia_rte->Export == "") { ?>
...
<?php
$asistencia_rte_view->Page_Terminate();
?>
