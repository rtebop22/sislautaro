...
$asistencia_alumnos = NULL;

//
// Table class for asistencia_alumnos
//
class casistencia_alumnos extends cTable {
	var $Dni;
	var $Apellidos_Nombres;
	var $Fecha;
	var $Usuario;
	var $Estado_Asistencia;
	...
	function __construct() {
		...
		$this->TableVar = 'asistencia_alumnos';
		$this->TableName = 'asistencia_alumnos';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`asistencia_alumnos`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// Dni
		$this->Dni = new cField('asistencia_alumnos', 'asistencia_alumnos', 'x_Dni', 'Dni', '`Dni`', '`Dni`', 3, -1, FALSE, '`Dni`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Dni->Sortable = TRUE; // Allow sort
		$this->Dni->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Dni'] = &$this->Dni;

		// Apellidos_Nombres
		$this->Apellidos_Nombres = new cField('asistencia_alumnos', 'asistencia_alumnos', 'x_Apellidos_Nombres', 'Apellidos_Nombres', '`Apellidos_Nombres`', '`Apellidos_Nombres`', 201, -1, FALSE, '`Apellidos_Nombres`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Apellidos_Nombres->Sortable = TRUE; // Allow sort
		$this->fields['Apellidos_Nombres'] = &$this->Apellidos_Nombres;

		// Fecha
		$this->Fecha = new cField('asistencia_alumnos', 'asistencia_alumnos', 'x_Fecha', 'Fecha', '`Fecha`', 'DATE_FORMAT(`Fecha`, \'\')', 133, -1, FALSE, '`Fecha`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Fecha->Sortable = TRUE; // Allow sort
		$this->fields['Fecha'] = &$this->Fecha;

		// Usuario
		$this->Usuario = new cField('asistencia_alumnos', 'asistencia_alumnos', 'x_Usuario', 'Usuario', '`Usuario`', '`Usuario`', 200, -1, FALSE, '`Usuario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Usuario->Sortable = TRUE; // Allow sort
		$this->fields['Usuario'] = &$this->Usuario;

		// Estado_Asistencia
		$this->Estado_Asistencia = new cField('asistencia_alumnos', 'asistencia_alumnos', 'x_Estado_Asistencia', 'Estado_Asistencia', '`Estado_Asistencia`', '`Estado_Asistencia`', 200, -1, FALSE, '`Estado_Asistencia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Estado_Asistencia->Sortable = TRUE; // Allow sort
		$this->fields['Estado_Asistencia'] = &$this->Estado_Asistencia;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`asistencia_alumnos`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "asistencia_alumnoslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("asistencia_alumnosview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("asistencia_alumnosview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "asistencia_alumnosadd.php?" . $this->UrlParm($parm);
		else
			$url = "asistencia_alumnosadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("asistencia_alumnosedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("asistencia_alumnosadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("asistencia_alumnosdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->Estado_Asistencia->EditValue = $this->Estado_Asistencia->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->Estado_Asistencia->Exportable) $Doc->ExportCaption($this->Estado_Asistencia);
				} else {
					if ($this->Dni->Exportable) $Doc->ExportCaption($this->Dni);
					if ($this->Fecha->Exportable) $Doc->ExportCaption($this->Fecha);
					if ($this->Usuario->Exportable) $Doc->ExportCaption($this->Usuario);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->Estado_Asistencia->Exportable) $Doc->ExportField($this->Estado_Asistencia);
					} else {
						if ($this->Dni->Exportable) $Doc->ExportField($this->Dni);
						if ($this->Fecha->Exportable) $Doc->ExportField($this->Fecha);
						if ($this->Usuario->Exportable) $Doc->ExportField($this->Usuario);
