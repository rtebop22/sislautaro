...
class catencion_equipos_view extends catencion_equipos {
	...
	function SetupOtherOptions() {
		...
		$links = "";
		if ($GLOBALS["detalle_atencion_grid"] && $GLOBALS["detalle_atencion_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'detalle_atencion')) {
		...
		if ($GLOBALS["detalle_atencion_grid"] && $GLOBALS["detalle_atencion_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'detalle_atencion')) {
		...
		if ($GLOBALS["detalle_atencion_grid"] && $GLOBALS["detalle_atencion_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'detalle_atencion')) {
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'detalle_atencion');
		if ($item->Visible) {
		...
		$links = "";
		if ($GLOBALS["atencion_para_st_grid"] && $GLOBALS["atencion_para_st_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'atencion_para_st')) {
		...
		if ($GLOBALS["atencion_para_st_grid"] && $GLOBALS["atencion_para_st_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'atencion_para_st')) {
		...
		if ($GLOBALS["atencion_para_st_grid"] && $GLOBALS["atencion_para_st_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'atencion_para_st')) {
		...
		$body = "<div class=\"btn-group\">" . $body . "</div>";
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'atencion_para_st');
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
		...
		$item = &$option->Add("detail_historial_atencion");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("historial_atencion", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["historial_atencion_grid"] && $GLOBALS["historial_atencion_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'historial_atencion')) {
		...
		if ($GLOBALS["historial_atencion_grid"] && $GLOBALS["historial_atencion_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'historial_atencion')) {
		...
		if ($GLOBALS["historial_atencion_grid"] && $GLOBALS["historial_atencion_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'historial_atencion')) {
		...
		$body = "<div class=\"btn-group\">" . $body . "</div>";
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'historial_atencion');
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
	...
	function ExportData() {
		...
		// Export detail records (historial_atencion)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("historial_atencion", explode(",", $this->getCurrentDetailTable()))) {
			global $historial_atencion;
			if (!isset($historial_atencion)) $historial_atencion = new chistorial_atencion;
			$rsdetail = $historial_atencion->LoadRs($historial_atencion->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("h"); // Change to horizontal
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					$Doc->ExportEmptyRow();
					$detailcnt = $rsdetail->RecordCount();
					$historial_atencion->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
				$Doc->SetStyle($ExportStyle); // Restore
				$rsdetail->Close();
			}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("historial_atencion", $DetailTblVar)) {
				...
				if ($GLOBALS["historial_atencion_grid"]->DetailView) {
					$GLOBALS["historial_atencion_grid"]->CurrentMode = "view";
...
<?php } ?>
<?php
	if (in_array("historial_atencion", explode(",", $atencion_equipos->getCurrentDetailTable())) && $historial_atencion->DetailView) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "historial_atencion") {
			$FirstActiveDetailTable = "historial_atencion";
		}
?>
	<div class="panel panel-default<?php echo $atencion_equipos_view->DetailPages->PageStyle("historial_atencion") ?>">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a class="panel-toggle" data-toggle="collapse" data-parent="#atencion_equipos_view_details" href="#tab_historial_atencion"><?php echo $Language->TablePhrase("historial_atencion", "TblCaption") ?></a>
			</h4>
		</div>
		<div class="panel-collapse collapse<?php echo $atencion_equipos_view->DetailPages->PageStyle("historial_atencion") ?>" id="tab_historial_atencion">
			<div class="panel-body">
...
<a href="actas/Atencion.php?Id_Atencion=<?php echo $atencion_equipos->Id_Atencion->ViewValue ?>" class="Estilo1">Comprobante de la Atenci&oacute;n</a>&nbsp;&nbsp; 
</div>
<?php if ($atencion_equipos->Export == "") { ?>
<script type="text/javascript">
