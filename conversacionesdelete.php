...
$conversaciones_delete = NULL; // Initialize page object first

class cconversaciones_delete extends cconversaciones {
	...
	// Page object name
	var $PageObjName = 'conversaciones_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("conversacioneslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in conversaciones class, conversacionesinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("conversacioneslist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($conversaciones_delete)) $conversaciones_delete = new cconversaciones_delete();

// Page init
$conversaciones_delete->Page_Init();

// Page main
$conversaciones_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$conversaciones_delete->Page_Render();
?>
...
var CurrentForm = fconversacionesdelete = new ew_Form("fconversacionesdelete", "delete");

// Form_CustomValidate event
fconversacionesdelete.Form_CustomValidate = 
...
fconversacionesdelete.ValidateRequired = true;
<?php } else { ?>
fconversacionesdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fconversacionesdelete.Lists["x_Usuario_1"] = {"LinkField":"x_Apellido_Nombre","Ajax":true,"AutoFill":false,"DisplayFields":["x_Apellido_Nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"referente_tecnico"};
...
<?php $conversaciones_delete->ShowPageHeader(); ?>
<?php
$conversaciones_delete->ShowMessage();
?>
<form name="fconversacionesdelete" id="fconversacionesdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($conversaciones_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $conversaciones_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="conversaciones">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($conversaciones_delete->RecKeys as $key) { ?>
...
<?php if ($conversaciones->Usuario_1->Visible) { // Usuario_1 ?>
		<th><span id="elh_conversaciones_Usuario_1" class="conversaciones_Usuario_1"><?php echo $conversaciones->Usuario_1->FldCaption() ?></span></th>
<?php } ?>
<?php if ($conversaciones->Usuario_2->Visible) { // Usuario_2 ?>
		<th><span id="elh_conversaciones_Usuario_2" class="conversaciones_Usuario_2"><?php echo $conversaciones->Usuario_2->FldCaption() ?></span></th>
<?php } ?>
<?php if ($conversaciones->Fecha_Hora->Visible) { // Fecha_Hora ?>
		<th><span id="elh_conversaciones_Fecha_Hora" class="conversaciones_Fecha_Hora"><?php echo $conversaciones->Fecha_Hora->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$conversaciones_delete->RecCnt = 0;
$i = 0;
while (!$conversaciones_delete->Recordset->EOF) {
	$conversaciones_delete->RecCnt++;
	$conversaciones_delete->RowCnt++;

	// Set row properties
	$conversaciones->ResetAttrs();
	$conversaciones->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$conversaciones_delete->LoadRowValues($conversaciones_delete->Recordset);

	// Render row
	$conversaciones_delete->RenderRow();
?>
	<tr<?php echo $conversaciones->RowAttributes() ?>>
<?php if ($conversaciones->Usuario_1->Visible) { // Usuario_1 ?>
		<td<?php echo $conversaciones->Usuario_1->CellAttributes() ?>>
<span id="el<?php echo $conversaciones_delete->RowCnt ?>_conversaciones_Usuario_1" class="conversaciones_Usuario_1">
...
<span id="el<?php echo $conversaciones_delete->RowCnt ?>_conversaciones_Usuario_2" class="conversaciones_Usuario_2">
...
<span id="el<?php echo $conversaciones_delete->RowCnt ?>_conversaciones_Fecha_Hora" class="conversaciones_Fecha_Hora">
...
<?php
	$conversaciones_delete->Recordset->MoveNext();
}
$conversaciones_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $conversaciones_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fconversacionesdelete.Init();
</script>
<?php
$conversaciones_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$conversaciones_delete->Page_Terminate();
?>
