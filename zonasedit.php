...
$zonas_edit = NULL; // Initialize page object first

class czonas_edit extends czonas {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->Id_Zona->CurrentValue == "") {
			$this->Page_Terminate("zonaslist.php"); // Invalid key, return to list
		}
...
// Create page object
if (!isset($zonas_edit)) $zonas_edit = new czonas_edit();

// Page init
$zonas_edit->Page_Init();

// Page main
$zonas_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$zonas_edit->Page_Render();
?>
...
var CurrentForm = fzonasedit = new ew_Form("fzonasedit", "edit");

// Validate form
fzonasedit.Validate = function() {
...
fzonasedit.Form_CustomValidate = 
...
fzonasedit.ValidateRequired = true;
<?php } else { ?>
fzonasedit.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $zonas_edit->ShowPageHeader(); ?>
<?php
$zonas_edit->ShowMessage();
?>
<form name="fzonasedit" id="fzonasedit" class="<?php echo $zonas_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($zonas_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $zonas_edit->Token ?>">
<?php } ?>
...
<?php if ($zonas->Id_Zona->Visible) { // Id_Zona ?>
	<div id="r_Id_Zona" class="form-group">
		<label id="elh_zonas_Id_Zona" class="col-sm-2 control-label ewLabel"><?php echo $zonas->Id_Zona->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $zonas->Id_Zona->CellAttributes() ?>>
...
<input type="hidden" data-table="zonas" data-field="x_Id_Zona" name="x_Id_Zona" id="x_Id_Zona" value="<?php echo ew_HtmlEncode($zonas->Id_Zona->CurrentValue) ?>">
<?php echo $zonas->Id_Zona->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $zonas_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
