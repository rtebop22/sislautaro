...
class cdato_establecimiento_add extends cdato_establecimiento {
	...
	function LoadDefaultValues() {
		...
		$this->Comparte_Edificio->OldValue = $this->Comparte_Edificio->CurrentValue;
		...
		$this->Geolocalizacion->OldValue = $this->Geolocalizacion->CurrentValue;
		...
		$this->Tiene_Programa->OldValue = $this->Tiene_Programa->CurrentValue;
		$this->Sector->CurrentValue = NULL;
		$this->Sector->OldValue = $this->Sector->CurrentValue;
		$this->Cantidad_Netbook_Conig->CurrentValue = NULL;
		$this->Cantidad_Netbook_Conig->OldValue = $this->Cantidad_Netbook_Conig->CurrentValue;
		...
		$this->Tipo_Zona->OldValue = $this->Tipo_Zona->CurrentValue;
	...
	function ValidateForm() {
		...
		if (in_array("datos_extras_escuela", $DetailTblVar) && $GLOBALS["datos_extras_escuela"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// Comparte_Edificio
		$this->Comparte_Edificio->SetDbValueDef($rsnew, $this->Comparte_Edificio->CurrentValue, NULL, FALSE);

		// Cantidad_Turnos
		$this->Cantidad_Turnos->SetDbValueDef($rsnew, $this->Cantidad_Turnos->CurrentValue, NULL, FALSE);

		// Geolocalizacion
		$this->Geolocalizacion->SetDbValueDef($rsnew, $this->Geolocalizacion->CurrentValue, NULL, FALSE);
		...
		// Tiene_Programa
		$this->Tiene_Programa->SetDbValueDef($rsnew, $this->Tiene_Programa->CurrentValue, NULL, FALSE);

		// Sector
		$this->Sector->SetDbValueDef($rsnew, $this->Sector->CurrentValue, NULL, FALSE);

		// Cantidad_Netbook_Conig
		$this->Cantidad_Netbook_Conig->SetDbValueDef($rsnew, $this->Cantidad_Netbook_Conig->CurrentValue, NULL, FALSE);

		// Cantidad_Netbook_Actuales
		$this->Cantidad_Netbook_Actuales->SetDbValueDef($rsnew, $this->Cantidad_Netbook_Actuales->CurrentValue, NULL, FALSE);

		// Id_Nivel
		$this->Id_Nivel->SetDbValueDef($rsnew, $this->Id_Nivel->CurrentValue, "", FALSE);

		// Id_Jornada
		$this->Id_Jornada->SetDbValueDef($rsnew, $this->Id_Jornada->CurrentValue, "", FALSE);

		// Tipo_Zona
		$this->Tipo_Zona->SetDbValueDef($rsnew, $this->Tipo_Zona->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			...
			if (in_array("datos_extras_escuela", $DetailTblVar) && $GLOBALS["datos_extras_escuela"]->DetailAdd) {
				$GLOBALS["datos_extras_escuela"]->Cue->setSessionValue($this->Cue->CurrentValue); // Set master key
				if (!isset($GLOBALS["datos_extras_escuela_grid"])) $GLOBALS["datos_extras_escuela_grid"] = new cdatos_extras_escuela_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "datos_extras_escuela"); // Load user level of detail table
				$AddRow = $GLOBALS["datos_extras_escuela_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["datos_extras_escuela"]->Cue->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("datos_extras_escuela", $DetailTblVar)) {
				...
				if ($GLOBALS["datos_extras_escuela_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["datos_extras_escuela_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["datos_extras_escuela_grid"]->CurrentMode = "add";
					$GLOBALS["datos_extras_escuela_grid"]->CurrentAction = "gridadd";
...
fdato_establecimientoadd.Lists["x_Comparte_Edificio"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fdato_establecimientoadd.Lists["x_Comparte_Edificio"].Options = <?php echo json_encode($dato_establecimiento->Comparte_Edificio->Options()) ?>;
fdato_establecimientoadd.Lists["x_Id_Tipo_Esc[]"] = {"LinkField":"x_Id_Tipo_Esc","Ajax":true,"AutoFill":false,"DisplayFields":["x_Detalle","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipo_escuela"};
fdato_establecimientoadd.Lists["x_Universo"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fdato_establecimientoadd.Lists["x_Universo"].Options = <?php echo json_encode($dato_establecimiento->Universo->Options()) ?>;
fdato_establecimientoadd.Lists["x_Tiene_Programa"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fdato_establecimientoadd.Lists["x_Tiene_Programa"].Options = <?php echo json_encode($dato_establecimiento->Tiene_Programa->Options()) ?>;
fdato_establecimientoadd.Lists["x_Sector"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fdato_establecimientoadd.Lists["x_Sector"].Options = <?php echo json_encode($dato_establecimiento->Sector->Options()) ?>;
fdato_establecimientoadd.Lists["x_Id_Nivel[]"] = {"LinkField":"x_Id_Nivel","Ajax":true,"AutoFill":false,"DisplayFields":["x_Detalle","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"nivel_educativo"};
fdato_establecimientoadd.Lists["x_Id_Jornada[]"] = {"LinkField":"x_Id_Jornada","Ajax":true,"AutoFill":false,"DisplayFields":["x_Detalle","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipo_jornada"};
fdato_establecimientoadd.Lists["x_Id_Estado_Esc"] = {"LinkField":"x_Id_Estado_Esc","Ajax":true,"AutoFill":false,"DisplayFields":["x_Detalle","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"estado_establecimiento"};
fdato_establecimientoadd.Lists["x_Id_Zona"] = {"LinkField":"x_Id_Zona","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"zonas"};
...
<input type="text" data-table="dato_establecimiento" data-field="x_Cue" data-page="1" name="x_Cue" id="x_Cue" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($dato_establecimiento->Cue->getPlaceHolder()) ?>" value="<?php echo $dato_establecimiento->Cue->EditValue ?>"<?php echo $dato_establecimiento->Cue->EditAttributes() ?>>
</span>
...
<?php } ?>
<?php
	if (in_array("datos_extras_escuela", explode(",", $dato_establecimiento->getCurrentDetailTable())) && $datos_extras_escuela->DetailAdd) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "datos_extras_escuela") {
			$FirstActiveDetailTable = "datos_extras_escuela";
		}
?>
	<div class="panel panel-default<?php echo $dato_establecimiento_add->DetailPages->PageStyle("datos_extras_escuela") ?>">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a class="panel-toggle" data-toggle="collapse" data-parent="#dato_establecimiento_add_details" href="#tab_datos_extras_escuela"><?php echo $Language->TablePhrase("datos_extras_escuela", "TblCaption") ?></a>
			</h4>
		</div>
		<div class="panel-collapse collapse<?php echo $dato_establecimiento_add->DetailPages->PageStyle("datos_extras_escuela") ?>" id="tab_datos_extras_escuela">
			<div class="panel-body">
