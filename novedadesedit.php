...
$novedades_edit = NULL; // Initialize page object first

class cnovedades_edit extends cnovedades {
	...
	// Page object name
	var $PageObjName = 'novedades_edit';
	...
	function Page_Main() {
		...
		if ($this->Id_Novedad->CurrentValue == "") {
			$this->Page_Terminate("novedadeslist.php"); // Invalid key, return to list
		}
	...
	function RenderRow() {
		...
		$this->Usuario->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->Archivos);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Archivos
			if ($this->Archivos->Visible && !$this->Archivos->ReadOnly && !$this->Archivos->Upload->KeepFile) {
...
// Create page object
if (!isset($novedades_edit)) $novedades_edit = new cnovedades_edit();

// Page init
$novedades_edit->Page_Init();

// Page main
$novedades_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$novedades_edit->Page_Render();
?>
...
var CurrentForm = fnovedadesedit = new ew_Form("fnovedadesedit", "edit");

// Validate form
fnovedadesedit.Validate = function() {
...
fnovedadesedit.Form_CustomValidate = 
...
fnovedadesedit.ValidateRequired = true;
<?php } else { ?>
fnovedadesedit.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $novedades_edit->ShowPageHeader(); ?>
<?php
$novedades_edit->ShowMessage();
?>
<form name="fnovedadesedit" id="fnovedadesedit" class="<?php echo $novedades_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($novedades_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $novedades_edit->Token ?>">
<?php } ?>
...
<?php if ($novedades->Id_Novedad->Visible) { // Id_Novedad ?>
	<div id="r_Id_Novedad" class="form-group">
		<label id="elh_novedades_Id_Novedad" class="col-sm-2 control-label ewLabel"><?php echo $novedades->Id_Novedad->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $novedades->Id_Novedad->CellAttributes() ?>>
...
<input type="hidden" data-table="novedades" data-field="x_Id_Novedad" name="x_Id_Novedad" id="x_Id_Novedad" value="<?php echo ew_HtmlEncode($novedades->Id_Novedad->CurrentValue) ?>">
<?php echo $novedades->Id_Novedad->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $novedades_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
