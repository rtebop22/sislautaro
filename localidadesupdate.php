...
$localidades_update = NULL; // Initialize page object first

class clocalidades_update extends clocalidades {
	...
	// Page object name
	var $PageObjName = 'localidades_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("localidadeslist.php"); // No records selected, return to list
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->Id_Localidad->CurrentValue = $sKeyFld;
		return TRUE;
	}
...
// Create page object
if (!isset($localidades_update)) $localidades_update = new clocalidades_update();

// Page init
$localidades_update->Page_Init();

// Page main
$localidades_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$localidades_update->Page_Render();
?>
...
var CurrentForm = flocalidadesupdate = new ew_Form("flocalidadesupdate", "update");

// Validate form
flocalidadesupdate.Validate = function() {
...
flocalidadesupdate.Form_CustomValidate = 
...
flocalidadesupdate.ValidateRequired = true;
<?php } else { ?>
flocalidadesupdate.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
flocalidadesupdate.Lists["x_Id_Departamento"] = {"LinkField":"x_Id_Departamento","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"departamento"};
...
<?php if (!$localidades_update->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $localidades_update->ShowPageHeader(); ?>
<?php
$localidades_update->ShowMessage();
?>
<form name="flocalidadesupdate" id="flocalidadesupdate" class="<?php echo $localidades_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($localidades_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $localidades_update->Token ?>">
<?php } ?>
...
<?php } ?>
<?php foreach ($localidades_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_localidadesupdate">
	<div class="checkbox">
...
<input type="checkbox" name="u_Nombre" id="u_Nombre" value="1"<?php echo ($localidades->Nombre->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $localidades->Nombre->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $localidades->Nombre->CellAttributes() ?>>
...
<?php echo $localidades->Nombre->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<input type="checkbox" name="u_Id_Departamento" id="u_Id_Departamento" value="1"<?php echo ($localidades->Id_Departamento->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $localidades->Id_Departamento->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $localidades->Id_Departamento->CellAttributes() ?>>
...
<?php echo $localidades->Id_Departamento->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if (!$localidades_update->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $localidades_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div>
	</div>
<?php } ?>
</div>
...
<script type="text/javascript">
flocalidadesupdate.Init();
</script>
<?php
$localidades_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$localidades_update->Page_Terminate();
?>
