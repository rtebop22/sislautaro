...
$chat_edit = NULL; // Initialize page object first

class cchat_edit extends cchat {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->Orden->CurrentValue == "") {
			$this->Page_Terminate("chatlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Nro_Conversacion
			$this->Nro_Conversacion->SetDbValueDef($rsnew, $this->Nro_Conversacion->CurrentValue, 0, $this->Nro_Conversacion->ReadOnly);
...
// Create page object
if (!isset($chat_edit)) $chat_edit = new cchat_edit();
...
var CurrentForm = fchatedit = new ew_Form("fchatedit", "edit");

// Validate form
fchatedit.Validate = function() {
...
fchatedit.ValidateRequired = true;
<?php } else { ?>
fchatedit.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $chat_edit->ShowPageHeader(); ?>
<?php
$chat_edit->ShowMessage();
?>
<form name="fchatedit" id="fchatedit" class="<?php echo $chat_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($chat_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $chat_edit->Token ?>">
<?php } ?>
...
<?php if ($chat->Orden->Visible) { // Orden ?>
	<div id="r_Orden" class="form-group">
		<label id="elh_chat_Orden" class="col-sm-2 control-label ewLabel"><?php echo $chat->Orden->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $chat->Orden->CellAttributes() ?>>
<span id="el_chat_Orden">
<span<?php echo $chat->Orden->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $chat->Orden->EditValue ?></p></span>
</span>
<input type="hidden" data-table="chat" data-field="x_Orden" name="x_Orden" id="x_Orden" value="<?php echo ew_HtmlEncode($chat->Orden->CurrentValue) ?>">
<?php echo $chat->Orden->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $chat_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
