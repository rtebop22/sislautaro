...
class catencion_para_st extends cTable {
	...
	function __construct() {
		...
		// NroSerie
		$this->NroSerie = new cField('atencion_para_st', 'atencion_para_st', 'x_NroSerie', 'NroSerie', '`NroSerie`', '`NroSerie`', 200, -1, FALSE, '`NroSerie`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->NroSerie->Sortable = TRUE; // Allow sort
		$this->fields['NroSerie'] = &$this->NroSerie;

		// Nro_Tiket
		$this->Nro_Tiket = new cField('atencion_para_st', 'atencion_para_st', 'x_Nro_Tiket', 'Nro_Tiket', '`Nro_Tiket`', '`Nro_Tiket`', 200, -1, FALSE, '`Nro_Tiket`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		$this->Referencia_Tipo_Retiro = new cField('atencion_para_st', 'atencion_para_st', 'x_Referencia_Tipo_Retiro', 'Referencia_Tipo_Retiro', '`Referencia_Tipo_Retiro`', '`Referencia_Tipo_Retiro`', 200, -1, FALSE, '`Referencia_Tipo_Retiro`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Referencia_Tipo_Retiro->Sortable = TRUE; // Allow sort
		$this->fields['Referencia_Tipo_Retiro'] = &$this->Referencia_Tipo_Retiro;

		// Fecha_Retiro
		$this->Fecha_Retiro = new cField('atencion_para_st', 'atencion_para_st', 'x_Fecha_Retiro', 'Fecha_Retiro', '`Fecha_Retiro`', 'DATE_FORMAT(`Fecha_Retiro`, \'\')', 133, 7, FALSE, '`Fecha_Retiro`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Fecha_Devolucion
		$this->Fecha_Devolucion = new cField('atencion_para_st', 'atencion_para_st', 'x_Fecha_Devolucion', 'Fecha_Devolucion', '`Fecha_Devolucion`', 'DATE_FORMAT(`Fecha_Devolucion`, \'\')', 133, 7, FALSE, '`Fecha_Devolucion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->Referencia_Tipo_Retiro->EditValue = $this->Referencia_Tipo_Retiro->CurrentValue;
		...
		$this->Fecha_Retiro->EditValue = ew_FormatDateTime($this->Fecha_Retiro->CurrentValue, 7);
		...
		$this->Fecha_Devolucion->EditValue = ew_FormatDateTime($this->Fecha_Devolucion->CurrentValue, 7);
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->Referencia_Tipo_Retiro->Exportable) $Doc->ExportCaption($this->Referencia_Tipo_Retiro);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->Referencia_Tipo_Retiro->Exportable) $Doc->ExportField($this->Referencia_Tipo_Retiro);
	...
	$Retiro=$rsnew["Id_Tipo_Retiro"];
	$Serie=$rsnew["NroSerie"];
	$usuario=CurrentUserName();
	$Fecha=ew_CurrentDate();

					//CONSULTO SI EXISTE TIKET CARGADO
	$sql1 = ew_ExecuteRow("SELECT Id_Atencion, Fecha_Devolucion FROM atencion_para_st WHERE NroSerie='$Serie' and Id_Atencion!=$Id_Atencion order by Id_Atencion desc limit 1");
	$ExisteSerie=$sql1["Id_Atencion"];
	$ExisteFecha=$sql1["Fecha_Devolucion"];
	if ($ExisteSerie>0){//sxiste pedido pendiente de devolucion
		if ($ExisteFecha==NULL){
			$Fecha_Actual_Devol=$rsnew["Fecha_Devolucion"];
				if ($Fecha_Actual_Devol==NULL){
					echo '<script language="javascript">alert("EXISTE UNA SOLICITUD ACTIVA QUE AUN NO VOLVIO DE SERVICIO TECNICO");</script>';						
					return FALSE;
				}else{
					echo '<script language="javascript">alert("tiene fecha No EXISTE UNA SOLICITUD ACTIVA");</script>';						
						if ($Retiro==1){
							...
							if ($Retiro==2){
								$QuienRetira='Retirado por el Correo';
							}else{
								$QuienRetira='Retirado por el Operativo de ST';
							}
							$Consulta1 = ew_Execute("UPDATE Equipos SET Id_Ubicacion=1, Id_Estado=2, Id_Sit_Estado=2, Ultima_Actualiz='$Fecha',User_Actualiz='$usuario' WHERE NroSerie='$Serie'");
							$Consulta2 = ew_Execute("INSERT INTO Observacion_Equipo (Detalle, Fecha_Actualizacion, NroSerie) VALUES ('El equipo se encuentra El Equipo fue retirado del establecimiento P/Serv. Tecnico', '$Fecha' ,'$Serie')");
							...
							$Consulta4 = ew_Execute("INSERT INTO Historial_Atencion (Detalle, Fecha_Actualizacion, NroSerie, Usuario, Id_Atencion) VALUES ('$QuienRetira', '$Fecha' ,'$Serie','$usuario',$Id_Atencion)");
							return FALSE;
						}
					}
				}else{
					echo '<script language="javascript">alert("No EXISTE serie");</script>';						
					if ($Retiro==1){
						$Consulta1 = ew_Execute("UPDATE Equipos SET Id_Ubicacion=1, Id_Estado=2, Id_Sit_Estado=2, Ultima_Actualiz='$Fecha',User_Actualiz='$usuario' WHERE NroSerie='$Serie'");
						$Consulta2 = ew_Execute("INSERT INTO Observacion_Equipo (Detalle, Fecha_Actualizacion, NroSerie) VALUES ('El equipo se encuentra Esperando retiro para Servicio Técnico', '$Fecha' ,'$Serie')");
						$Consulta1 = ew_Execute("UPDATE Detalle_Atencion SET Id_Estado_Atenc=1, Fecha_Actualizacion='$Fecha' WHERE Id_Atencion=$Id_Atencion");
					...
					echo '<script language="javascript">alert("NO HAY SERIE");</script>';
					return FALSE;
	}
	}
	...
	function Row_Updating($rsold, &$rsnew) {
					...
					$usuario=CurrentUserName();
					$Fecha=ew_CurrentDate();
					$FechaDevol=$rsnew["Fecha_Devolucion"];
					$Tiket=$rsnew["Nro_Tiket"];

	//Consultamos si es retiro o devolucion
	$sql1 = ew_ExecuteRow("SELECT Fecha_Devolucion FROM atencion_para_st WHERE Id_Atencion=$Id_Atencion");
	$ExisteFecha=$sql1["Fecha_Devolucion"];
		echo '<script language="javascript">alert("',$ExisteFecha,'");</script>';
	if ($ExisteFecha>0){
		echo '<script language="javascript">alert("existe fecha");</script>';
		$Consulta1 = ew_Execute("UPDATE Equipos SET Id_Ubicacion=1, Id_Estado=2, Id_Sit_Estado=12, Ultima_Actualiz='$Fecha',User_Actualiz='$usuario' WHERE NroSerie='$Serie'");
	...
	if ($FechaDevol!=NULL){
		...
		return TRUE;
					...
					//CONSULTO SI EXISTE TIKET CARGADO
					$sql = ew_ExecuteRow("SELECT Id_Atencion, Nro_Tiket FROM atencion_para_st WHERE Nro_Tiket='$Tiket' and Id_Atencion!=$Id_Atencion ");
					$NroAtencion=$sql["Id_Atencion"];
					$TiketBD=$sql["Nro_Tiket"];
					if ($NroAtencion>0){//EXISTE
						if ($TiketBD=='PENDIENTE'){
							$sql1 = ew_ExecuteRow("SELECT Id_Atencion, Fecha_Devolucion FROM atencion_para_st WHERE NroSerie='$Serie' and Id_Atencion!=$Id_Atencion order by Id_Atencion desc limit 1");
							$ExisteSerie=$sql1["Id_Atencion"];
							$ExisteFecha=$sql1["Fecha_Devolucion"];
							if ($ExisteSerie>0){//sxiste pedido pendiente de devolucion
							...
							echo '<script language="javascript">alert("EXISTE OTRA SOLICITUD CON EL MISMO NUMERO DE TIKET");</script>';
							return FALSE;
						}	
					}else{
						$sql1 = ew_ExecuteRow("SELECT Id_Atencion, Fecha_Devolucion FROM atencion_para_st WHERE NroSerie='$Serie' and Id_Atencion!=$Id_Atencion order by Id_Atencion desc limit 1");
