...
$tutores_add = NULL; // Initialize page object first

class ctutores_add extends ctutores {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["Dni_Tutor"] != "") {
				...
				$this->setKey("Dni_Tutor", $this->Dni_Tutor->CurrentValue); // Set up key
			} else {
				$this->setKey("Dni_Tutor", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tutoresview.php")
	...
	function LoadDefaultValues() {
		...
		$this->Id_Estado_Civil->OldValue = $this->Id_Estado_Civil->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("observacion_tutor", $DetailTblVar) && $GLOBALS["observacion_tutor"]->DetailAdd) {
				$GLOBALS["observacion_tutor"]->Dni_Tutor->setSessionValue($this->Dni_Tutor->CurrentValue); // Set master key
				if (!isset($GLOBALS["observacion_tutor_grid"])) $GLOBALS["observacion_tutor_grid"] = new cobservacion_tutor_grid(); // Get detail page object
				$AddRow = $GLOBALS["observacion_tutor_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["observacion_tutor"]->Dni_Tutor->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("observacion_tutor", $DetailTblVar)) {
				...
				if ($GLOBALS["observacion_tutor_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["observacion_tutor_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["observacion_tutor_grid"]->CurrentMode = "add";
					$GLOBALS["observacion_tutor_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($tutores_add)) $tutores_add = new ctutores_add();

// Page init
$tutores_add->Page_Init();

// Page main
$tutores_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tutores_add->Page_Render();
?>
...
var CurrentForm = ftutoresadd = new ew_Form("ftutoresadd", "add");

// Validate form
ftutoresadd.Validate = function() {
...
ftutoresadd.Form_CustomValidate = 
...
ftutoresadd.ValidateRequired = true;
<?php } else { ?>
ftutoresadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $tutores_add->ShowPageHeader(); ?>
<?php
$tutores_add->ShowMessage();
?>
<form name="ftutoresadd" id="ftutoresadd" class="<?php echo $tutores_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tutores_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tutores_add->Token ?>">
<?php } ?>
...
<input type="hidden" id="x_Dni_Tutor" name="x_Dni_Tutor" value="<?php echo ew_HtmlEncode($tutores->Dni_Tutor->CurrentValue) ?>">
<?php } else { ?>
<span id="el_tutores_Dni_Tutor">
...
<script type="text/javascript">
ew_CreateCalendar("ftutoresadd", "x_Fecha_Nac", 7);
</script>
<?php } ?>
</span>
...
<?php if ($tutores->Id_Ocupacion->Visible) { // Id_Ocupacion ?>
	<div id="r_Id_Ocupacion" class="form-group">
		<label id="elh_tutores_Id_Ocupacion" class="col-sm-2 control-label ewLabel"><?php echo $tutores->Id_Ocupacion->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
...
<script type="text/javascript">
ftutoresadd.CreateAutoSuggest({"id":"x_Id_Ocupacion","forceSelect":false});
</script>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tutores_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
