...
$modelo_add = NULL; // Initialize page object first

class cmodelo_add extends cmodelo {
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("modelolist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "modelolist.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to list page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "modeloview.php")
...
// Create page object
if (!isset($modelo_add)) $modelo_add = new cmodelo_add();

// Page init
$modelo_add->Page_Init();

// Page main
$modelo_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$modelo_add->Page_Render();
?>
...
var CurrentForm = fmodeloadd = new ew_Form("fmodeloadd", "add");

// Validate form
fmodeloadd.Validate = function() {
...
fmodeloadd.Form_CustomValidate = 
...
fmodeloadd.ValidateRequired = true;
<?php } else { ?>
fmodeloadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmodeloadd.Lists["x_Id_Marca"] = {"LinkField":"x_Id_Marca","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"marca"};
...
<?php } ?>
<?php $modelo_add->ShowPageHeader(); ?>
<?php
$modelo_add->ShowMessage();
?>
<form name="fmodeloadd" id="fmodeloadd" class="<?php echo $modelo_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($modelo_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $modelo_add->Token ?>">
<?php } ?>
...
<?php if ($modelo->Descripcion->Visible) { // Descripcion ?>
	<div id="r_Descripcion" class="form-group">
		<label id="elh_modelo_Descripcion" for="x_Descripcion" class="col-sm-2 control-label ewLabel"><?php echo $modelo->Descripcion->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $modelo->Descripcion->CellAttributes() ?>>
...
<?php echo $modelo->Descripcion->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($modelo->Id_Marca->Visible) { // Id_Marca ?>
	<div id="r_Id_Marca" class="form-group">
		<label id="elh_modelo_Id_Marca" for="x_Id_Marca" class="col-sm-2 control-label ewLabel"><?php echo $modelo->Id_Marca->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $modelo->Id_Marca->CellAttributes() ?>>
...
<?php echo $modelo->Id_Marca->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$modelo_add->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $modelo_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
