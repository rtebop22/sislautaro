...
class cusuarios_add extends cusuarios {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["User"] != "") {
				...
				$this->setKey("User", $this->User->CurrentValue); // Set up key
			} else {
				$this->setKey("User", ""); // Clear key
	...
	function RenderRow() {
		...
		$this->Id_Grupo->ViewCustomAttributes = "";
			...
			$sSqlWrk = "SELECT `User`, `User` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `usuarios`";
			$sWhereWrk = "";
			...
			$sFilterWrk = "";
			$sFilterWrk = $GLOBALS["usuarios"]->AddUserIDFilter("");
			$sSqlWrk = "SELECT `User`, `User` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `usuarios`";
			$sWhereWrk = "";
			$this->User->LookupFilters = array();
			ew_AddFilter($sWhereWrk, $sFilterWrk);
...
fusuariosadd.Lists["x_Id_Grupo"] = {"LinkField":"x_Id_Grupo","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"grupo_user"};
...
<select data-table="usuarios" data-field="x_User" data-value-separator="<?php echo $usuarios->User->DisplayValueSeparatorAttribute() ?>" id="x_User" name="x_User"<?php echo $usuarios->User->EditAttributes() ?>>
<?php echo $usuarios->User->SelectOptionListHtml("x_User") ?>
</select>
</span>
<?php } elseif (!$Security->IsAdmin() && $Security->IsLoggedIn() && !$usuarios->UserIDAllow("add")) { // Non system admin ?>
<span id="el_usuarios_User">
<select data-table="usuarios" data-field="x_User" data-value-separator="<?php echo $usuarios->User->DisplayValueSeparatorAttribute() ?>" id="x_User" name="x_User"<?php echo $usuarios->User->EditAttributes() ?>>
<?php echo $usuarios->User->SelectOptionListHtml("x_User") ?>
</select>
