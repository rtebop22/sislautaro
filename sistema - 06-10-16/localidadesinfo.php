...
class clocalidades extends cTable {
	...
	function __construct() {
		...
		// Id_Localidad
		$this->Id_Localidad = new cField('localidades', 'localidades', 'x_Id_Localidad', 'Id_Localidad', '`Id_Localidad`', '`Id_Localidad`', 3, -1, FALSE, '`Id_Localidad`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		...
		// Nombre
		$this->Nombre = new cField('localidades', 'localidades', 'x_Nombre', 'Nombre', '`Nombre`', '`Nombre`', 200, -1, FALSE, '`Nombre`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Nombre->Sortable = TRUE; // Allow sort
		$this->fields['Nombre'] = &$this->Nombre;

		// Id_Departamento
		$this->Id_Departamento = new cField('localidades', 'localidades', 'x_Id_Departamento', 'Id_Departamento', '`Id_Departamento`', '`Id_Departamento`', 3, -1, FALSE, '`Id_Departamento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('Id_Localidad', $rs))
				ew_AddFilter($where, ew_QuotedName('Id_Localidad', $this->DBID) . '=' . ew_QuotedValue($rs['Id_Localidad'], $this->Id_Localidad->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->Id_Localidad->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@Id_Localidad@", ew_AdjustSql($this->Id_Localidad->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "Id_Localidad:" . ew_VarToJson($this->Id_Localidad->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->Id_Localidad->CurrentValue)) {
			$sUrl .= "Id_Localidad=" . urlencode($this->Id_Localidad->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["Id_Localidad"]))
				$arKeys[] = ew_StripSlashes($_POST["Id_Localidad"]);
			elseif (isset($_GET["Id_Localidad"]))
				$arKeys[] = ew_StripSlashes($_GET["Id_Localidad"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->Id_Localidad->CurrentValue = $key;
