...
class cusuarios extends cTable {
	...
	function __construct() {
		...
		// User
		$this->User = new cField('usuarios', 'usuarios', 'x_User', 'User', '`User`', '`User`', 200, -1, FALSE, '`User`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->User->Sortable = TRUE; // Allow sort
		$this->fields['User'] = &$this->User;

		// Pass
		$this->Pass = new cField('usuarios', 'usuarios', 'x_Pass', 'Pass', '`Pass`', '`Pass`', 200, -1, FALSE, '`Pass`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'PASSWORD');
		$this->Pass->Sortable = TRUE; // Allow sort
		$this->fields['Pass'] = &$this->Pass;

		// Id_Grupo
		$this->Id_Grupo = new cField('usuarios', 'usuarios', 'x_Id_Grupo', 'Id_Grupo', '`Id_Grupo`', '`Id_Grupo`', 3, -1, FALSE, '`Id_Grupo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('User', $rs))
				ew_AddFilter($where, ew_QuotedName('User', $this->DBID) . '=' . ew_QuotedValue($rs['User'], $this->User->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@User@", ew_AdjustSql($this->User->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "User:" . ew_VarToJson($this->User->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if (!is_null($this->User->CurrentValue)) {
			$sUrl .= "User=" . urlencode($this->User->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["User"]))
				$arKeys[] = ew_StripSlashes($_POST["User"]);
			elseif (isset($_GET["User"]))
				$arKeys[] = ew_StripSlashes($_GET["User"]);
			else
   ...
   // Common render codes
	...
	function UserIDFilter($userid) {
		$sUserIDFilter = '`User` = ' . ew_QuotedValue($userid, EW_DATATYPE_STRING, EW_USER_TABLE_DBID);
		$sParentUserIDFilter = '`User` IN (SELECT `User` FROM ' . "`usuarios`" . ' WHERE `User` = ' . ew_QuotedValue($userid, EW_DATATYPE_STRING, EW_USER_TABLE_DBID) . ')';
