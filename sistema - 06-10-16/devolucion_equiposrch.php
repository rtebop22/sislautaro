...
class cdevolucion_equipo_search extends cdevolucion_equipo {
	...
	function RenderRow() {
		...
		$this->Devuelve_Cargador->ViewCustomAttributes = "";
			...
			if (trim(strval($this->Dni->AdvancedSearch->SearchValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
			...
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				...
				$this->Dni->AdvancedSearch->ViewValue = $this->Dni->DisplayValue($arwrk);
			} else {
				$this->Dni->AdvancedSearch->ViewValue = $Language->Phrase("PleaseSelect");
			}
			...
			if (trim(strval($this->Dni_Tutor->AdvancedSearch->SearchValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
			...
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				...
				$arwrk[1] = ew_HtmlEncode($rswrk->fields('DispFld'));
				$arwrk[2] = ew_HtmlEncode($rswrk->fields('Disp2Fld'));
				$this->Dni_Tutor->AdvancedSearch->ViewValue = $this->Dni_Tutor->DisplayValue($arwrk);
			} else {
				$this->Dni_Tutor->AdvancedSearch->ViewValue = $Language->Phrase("PleaseSelect");
			}
			$arwrk = ($rswrk) ? $rswrk->GetRows() : array();
			if ($rswrk) $rswrk->Close();
			...
			$this->Fecha_Devolucion->EditValue = ew_HtmlEncode($this->Fecha_Devolucion->AdvancedSearch->SearchValue);
...
fdevolucion_equiposearch.Lists["x_Admin_Que_Recibe"] = {"LinkField":"x_DniRte","Ajax":true,"AutoFill":false,"DisplayFields":["x_Apelldio_Nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"referente_tecnico"};
...
fdevolucion_equiposearch.Lists["x_Id_Estado_Devol"] = {"LinkField":"x_Id_Estado_Devol","Ajax":true,"AutoFill":false,"DisplayFields":["x_Detalle","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"estado_equipo_devuleto"};
...
<?php if ($devolucion_equipo->Dni->Visible) { // Dni ?>
	<div id="r_Dni" class="form-group">
		<label for="x_Dni" class="<?php echo $devolucion_equipo_search->SearchLabelClass ?>"><span id="elh_devolucion_equipo_Dni"><?php echo $devolucion_equipo->Dni->FldCaption() ?></span>	
...
<?php if ($devolucion_equipo->NroSerie->Visible) { // NroSerie ?>
	<div id="r_NroSerie" class="form-group">
		<label for="x_NroSerie" class="<?php echo $devolucion_equipo_search->SearchLabelClass ?>"><span id="elh_devolucion_equipo_NroSerie"><?php echo $devolucion_equipo->NroSerie->FldCaption() ?></span>	
...
<?php if ($devolucion_equipo->Dni_Tutor->Visible) { // Dni_Tutor ?>
	<div id="r_Dni_Tutor" class="form-group">
		<label for="x_Dni_Tutor" class="<?php echo $devolucion_equipo_search->SearchLabelClass ?>"><span id="elh_devolucion_equipo_Dni_Tutor"><?php echo $devolucion_equipo->Dni_Tutor->FldCaption() ?></span>	
