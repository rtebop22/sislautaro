...
$estado_actual_legajo_persona_grid = NULL; // Initialize page object first

class cestado_actual_legajo_persona_grid extends cestado_actual_legajo_persona {
	...
	// Page object name
	var $PageObjName = 'estado_actual_legajo_persona_grid';

	// Grid form hidden field names
	var $FormName = 'festado_actual_legajo_personagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["estado_actual_legajo_persona"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->Dni->CurrentValue = strval($arKeys[0]); // Dni
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
