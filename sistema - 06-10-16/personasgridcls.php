...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function RenderRow() {
		...
		// Id_Nacionalidad
		if (strval($this->Id_Nacionalidad->CurrentValue) <> "") {
			$sFilterWrk = "`Id_Nacionalidad`" . ew_SearchString("=", $this->Id_Nacionalidad->CurrentValue, EW_DATATYPE_NUMBER, "");
		$sSqlWrk = "SELECT `Id_Nacionalidad`, `Nombre` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `nacionalidades`";
		$sWhereWrk = "";
		$this->Id_Nacionalidad->LookupFilters = array();
		ew_AddFilter($sWhereWrk, $sFilterWrk);
		$this->Lookup_Selecting($this->Id_Nacionalidad, $sWhereWrk); // Call Lookup selecting
		if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			$rswrk = Conn()->Execute($sSqlWrk);
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				$arwrk = array();
				$arwrk[1] = $rswrk->fields('DispFld');
				$this->Id_Nacionalidad->ViewValue = $this->Id_Nacionalidad->DisplayValue($arwrk);
				$rswrk->Close();
			} else {
				$this->Id_Nacionalidad->ViewValue = $this->Id_Nacionalidad->CurrentValue;
			}
		} else {
			$this->Id_Nacionalidad->ViewValue = NULL;
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Id_Cargo
			$this->Id_Cargo->SetDbValueDef($rsnew, $this->Id_Cargo->CurrentValue, "", $this->Id_Cargo->ReadOnly);
	...
	function AddRow($rsold = NULL) {
		...
		// Id_Cargo
		$this->Id_Cargo->SetDbValueDef($rsnew, $this->Id_Cargo->CurrentValue, "", FALSE);
