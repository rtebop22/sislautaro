...
$tutores_delete = NULL; // Initialize page object first

class ctutores_delete extends ctutores {
	...
	// Page object name
	var $PageObjName = 'tutores_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("tutoreslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in tutores class, tutoresinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("tutoreslist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($tutores_delete)) $tutores_delete = new ctutores_delete();

// Page init
$tutores_delete->Page_Init();

// Page main
$tutores_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tutores_delete->Page_Render();
?>
...
var CurrentForm = ftutoresdelete = new ew_Form("ftutoresdelete", "delete");

// Form_CustomValidate event
ftutoresdelete.Form_CustomValidate = 
...
ftutoresdelete.ValidateRequired = true;
<?php } else { ?>
ftutoresdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftutoresdelete.Lists["x_MasHijos"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftutoresdelete.Lists["x_MasHijos"].Options = <?php echo json_encode($tutores->MasHijos->Options()) ?>;
ftutoresdelete.Lists["x_Id_Relacion"] = {"LinkField":"x_Id_Relacion","Ajax":true,"AutoFill":false,"DisplayFields":["x_Desripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipo_relacion_alumno_tutor"};
ftutoresdelete.Lists["x_Id_Departamento"] = {"LinkField":"x_Id_Departamento","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":[],"ChildFields":["x_Id_Localidad"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"departamento"};
...
<?php $tutores_delete->ShowPageHeader(); ?>
<?php
$tutores_delete->ShowMessage();
?>
<form name="ftutoresdelete" id="ftutoresdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tutores_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tutores_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tutores">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tutores_delete->RecKeys as $key) { ?>
...
<?php if ($tutores->Dni_Tutor->Visible) { // Dni_Tutor ?>
		<th><span id="elh_tutores_Dni_Tutor" class="tutores_Dni_Tutor"><?php echo $tutores->Dni_Tutor->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tutores->Apellidos_Nombres->Visible) { // Apellidos_Nombres ?>
		<th><span id="elh_tutores_Apellidos_Nombres" class="tutores_Apellidos_Nombres"><?php echo $tutores->Apellidos_Nombres->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tutores->Domicilio->Visible) { // Domicilio ?>
		<th><span id="elh_tutores_Domicilio" class="tutores_Domicilio"><?php echo $tutores->Domicilio->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tutores->Tel_Contacto->Visible) { // Tel_Contacto ?>
		<th><span id="elh_tutores_Tel_Contacto" class="tutores_Tel_Contacto"><?php echo $tutores->Tel_Contacto->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tutores->Cuil->Visible) { // Cuil ?>
		<th><span id="elh_tutores_Cuil" class="tutores_Cuil"><?php echo $tutores->Cuil->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tutores->MasHijos->Visible) { // MasHijos ?>
		<th><span id="elh_tutores_MasHijos" class="tutores_MasHijos"><?php echo $tutores->MasHijos->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tutores->Id_Relacion->Visible) { // Id_Relacion ?>
		<th><span id="elh_tutores_Id_Relacion" class="tutores_Id_Relacion"><?php echo $tutores->Id_Relacion->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tutores->Id_Departamento->Visible) { // Id_Departamento ?>
		<th><span id="elh_tutores_Id_Departamento" class="tutores_Id_Departamento"><?php echo $tutores->Id_Departamento->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$tutores_delete->RecCnt = 0;
$i = 0;
while (!$tutores_delete->Recordset->EOF) {
	$tutores_delete->RecCnt++;
	$tutores_delete->RowCnt++;

	// Set row properties
	$tutores->ResetAttrs();
	$tutores->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tutores_delete->LoadRowValues($tutores_delete->Recordset);

	// Render row
	$tutores_delete->RenderRow();
?>
	<tr<?php echo $tutores->RowAttributes() ?>>
<?php if ($tutores->Dni_Tutor->Visible) { // Dni_Tutor ?>
		<td<?php echo $tutores->Dni_Tutor->CellAttributes() ?>>
<span id="el<?php echo $tutores_delete->RowCnt ?>_tutores_Dni_Tutor" class="tutores_Dni_Tutor">
...
<span id="el<?php echo $tutores_delete->RowCnt ?>_tutores_Apellidos_Nombres" class="tutores_Apellidos_Nombres">
...
<span id="el<?php echo $tutores_delete->RowCnt ?>_tutores_Domicilio" class="tutores_Domicilio">
...
<span id="el<?php echo $tutores_delete->RowCnt ?>_tutores_Tel_Contacto" class="tutores_Tel_Contacto">
...
<span id="el<?php echo $tutores_delete->RowCnt ?>_tutores_Cuil" class="tutores_Cuil">
...
<span id="el<?php echo $tutores_delete->RowCnt ?>_tutores_MasHijos" class="tutores_MasHijos">
...
<span id="el<?php echo $tutores_delete->RowCnt ?>_tutores_Id_Relacion" class="tutores_Id_Relacion">
...
<span id="el<?php echo $tutores_delete->RowCnt ?>_tutores_Id_Departamento" class="tutores_Id_Departamento">
...
<?php
	$tutores_delete->Recordset->MoveNext();
}
$tutores_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tutores_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftutoresdelete.Init();
