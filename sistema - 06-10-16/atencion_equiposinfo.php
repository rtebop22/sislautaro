...
class catencion_equipos extends cTable {
	...
	function __construct() {
		...
		// NroSerie
		$this->NroSerie = new cField('atencion_equipos', 'atencion_equipos', 'x_NroSerie', 'NroSerie', '`NroSerie`', '`NroSerie`', 200, -1, FALSE, '`NroSerie`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function Delete(&$rs, $where = "", $curfilter = TRUE) {
		...
		$rscascade = $GLOBALS["detalle_atencion"]->LoadRs("`Id_Atencion` = " . ew_QuotedValue($rs['Id_Atencion'], EW_DATATYPE_NUMBER, "DB")); 
		...
		$rscascade = $GLOBALS["atencion_para_st"]->LoadRs("`Id_Atencion` = " . ew_QuotedValue($rs['Id_Atencion'], EW_DATATYPE_NUMBER, "DB")); 
		while ($rscascade && !$rscascade->EOF) {
