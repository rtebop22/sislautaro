...
$autoridades_escolares_add = NULL; // Initialize page object first

class cautoridades_escolares_add extends cautoridades_escolares {
	...
	// Page object name
	var $PageObjName = 'autoridades_escolares_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "autoridades_escolaresview.php")
...
// Create page object
if (!isset($autoridades_escolares_add)) $autoridades_escolares_add = new cautoridades_escolares_add();

// Page init
$autoridades_escolares_add->Page_Init();

// Page main
$autoridades_escolares_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$autoridades_escolares_add->Page_Render();
?>
...
var CurrentForm = fautoridades_escolaresadd = new ew_Form("fautoridades_escolaresadd", "add");

// Validate form
fautoridades_escolaresadd.Validate = function() {
...
fautoridades_escolaresadd.Form_CustomValidate = 
...
fautoridades_escolaresadd.ValidateRequired = true;
<?php } else { ?>
fautoridades_escolaresadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fautoridades_escolaresadd.Lists["x_Id_Cargo"] = {"LinkField":"x_Id_Cargo","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cargo_autoridad"};
fautoridades_escolaresadd.Lists["x_Id_Turno"] = {"LinkField":"x_Id_Turno","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"turno"};
...
<?php if (!$autoridades_escolares_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $autoridades_escolares_add->ShowPageHeader(); ?>
<?php
$autoridades_escolares_add->ShowMessage();
?>
<form name="fautoridades_escolaresadd" id="fautoridades_escolaresadd" class="<?php echo $autoridades_escolares_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($autoridades_escolares_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $autoridades_escolares_add->Token ?>">
<?php } ?>
...
<?php } ?>
<div>
<?php if ($autoridades_escolares->Id_Autoridad->Visible) { // Id_Autoridad ?>
	<div id="r_Id_Autoridad" class="form-group">
		<label id="elh_autoridades_escolares_Id_Autoridad" class="col-sm-2 control-label ewLabel"><?php echo $autoridades_escolares->Id_Autoridad->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $autoridades_escolares->Id_Autoridad->CellAttributes() ?>><?php echo $autoridades_escolares->Id_Autoridad->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if (strval($autoridades_escolares->Cue->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_Cue" id="x_Cue" value="<?php echo ew_HtmlEncode(strval($autoridades_escolares->Cue->getSessionValue())) ?>">
<?php } ?>
<?php if (!$autoridades_escolares_add->IsModal) { ?>
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $autoridades_escolares_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fautoridades_escolaresadd.Init();
</script>
<?php
$autoridades_escolares_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$autoridades_escolares_add->Page_Terminate();
?>
