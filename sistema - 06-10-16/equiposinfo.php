...
class cequipos extends cTable {
	...
	function __construct() {
		...
		// User_Actualiz
		$this->User_Actualiz = new cField('equipos', 'equipos', 'x_User_Actualiz', 'User_Actualiz', '`User_Actualiz`', '`User_Actualiz`', 200, 7, FALSE, '`User_Actualiz`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->User_Actualiz->Sortable = TRUE; // Allow sort
		$this->fields['User_Actualiz'] = &$this->User_Actualiz;

		// Ultima_Actualiz
		$this->Ultima_Actualiz = new cField('equipos', 'equipos', 'x_Ultima_Actualiz', 'Ultima_Actualiz', '`Ultima_Actualiz`', '`Ultima_Actualiz`', 200, -1, FALSE, '`Ultima_Actualiz`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->Ultima_Actualiz->Exportable) $Doc->ExportCaption($this->Ultima_Actualiz);
				} else {
					if ($this->NroSerie->Exportable) $Doc->ExportCaption($this->NroSerie);
					if ($this->NroMac->Exportable) $Doc->ExportCaption($this->NroMac);
					if ($this->Id_Ubicacion->Exportable) $Doc->ExportCaption($this->Id_Ubicacion);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->Ultima_Actualiz->Exportable) $Doc->ExportField($this->Ultima_Actualiz);
					} else {
						if ($this->NroSerie->Exportable) $Doc->ExportField($this->NroSerie);
						if ($this->NroMac->Exportable) $Doc->ExportField($this->NroMac);
						if ($this->Id_Ubicacion->Exportable) $Doc->ExportField($this->Id_Ubicacion);
