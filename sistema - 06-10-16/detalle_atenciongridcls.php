...
$detalle_atencion_grid = NULL; // Initialize page object first

class cdetalle_atencion_grid extends cdetalle_atencion {
	...
	// Page object name
	var $PageObjName = 'detalle_atencion_grid';

	// Grid form hidden field names
	var $FormName = 'fdetalle_atenciongrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["detalle_atencion"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetRecordKey(&$key, $rs) {
		...
		if ($key <> "") $key .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
		$key .= $rs->fields('Id_Detalle_Atencion');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				...
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->Id_Detalle_Atencion->CurrentValue = strval($arKeys[1]); // Id_Detalle_Atencion
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
