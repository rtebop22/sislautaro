...
class cpersonas_edit extends cpersonas {
	...
	function ValidateForm() {
		...
		if (in_array("estado_actual_legajo_persona", $DetailTblVar) && $GLOBALS["estado_actual_legajo_persona"]->DetailEdit) {
		...
		if (in_array("materias_adeudadas", $DetailTblVar) && $GLOBALS["materias_adeudadas"]->DetailEdit) {
		...
		if (in_array("observacion_persona", $DetailTblVar) && $GLOBALS["observacion_persona"]->DetailEdit) {
		...
		if (in_array("equipos", $DetailTblVar) && $GLOBALS["equipos"]->DetailEdit) {
		...
		if (in_array("tutores", $DetailTblVar) && $GLOBALS["tutores"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Fecha_Nac
			$this->Fecha_Nac->SetDbValueDef($rsnew, $this->Fecha_Nac->CurrentValue, NULL, $this->Fecha_Nac->ReadOnly);

			// Lugar_Nacimiento
			$this->Lugar_Nacimiento->SetDbValueDef($rsnew, $this->Lugar_Nacimiento->CurrentValue, NULL, $this->Lugar_Nacimiento->ReadOnly);

			// Cod_Postal
			$this->Cod_Postal->SetDbValueDef($rsnew, $this->Cod_Postal->CurrentValue, NULL, $this->Cod_Postal->ReadOnly);

			// Repitente
			$this->Repitente->SetDbValueDef($rsnew, $this->Repitente->CurrentValue, NULL, $this->Repitente->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("estado_actual_legajo_persona", $DetailTblVar) && $GLOBALS["estado_actual_legajo_persona"]->DetailEdit) {
						if (!isset($GLOBALS["estado_actual_legajo_persona_grid"])) $GLOBALS["estado_actual_legajo_persona_grid"] = new cestado_actual_legajo_persona_grid(); // Get detail page object
						$EditRow = $GLOBALS["estado_actual_legajo_persona_grid"]->GridUpdate();
					}
				}
				if ($EditRow) {
					if (in_array("materias_adeudadas", $DetailTblVar) && $GLOBALS["materias_adeudadas"]->DetailEdit) {
						if (!isset($GLOBALS["materias_adeudadas_grid"])) $GLOBALS["materias_adeudadas_grid"] = new cmaterias_adeudadas_grid(); // Get detail page object
						$EditRow = $GLOBALS["materias_adeudadas_grid"]->GridUpdate();
					}
				}
				if ($EditRow) {
					if (in_array("observacion_persona", $DetailTblVar) && $GLOBALS["observacion_persona"]->DetailEdit) {
						if (!isset($GLOBALS["observacion_persona_grid"])) $GLOBALS["observacion_persona_grid"] = new cobservacion_persona_grid(); // Get detail page object
						$EditRow = $GLOBALS["observacion_persona_grid"]->GridUpdate();
					}
				}
				if ($EditRow) {
					if (in_array("equipos", $DetailTblVar) && $GLOBALS["equipos"]->DetailEdit) {
						if (!isset($GLOBALS["equipos_grid"])) $GLOBALS["equipos_grid"] = new cequipos_grid(); // Get detail page object
						$EditRow = $GLOBALS["equipos_grid"]->GridUpdate();
					}
				}
				if ($EditRow) {
					if (in_array("tutores", $DetailTblVar) && $GLOBALS["tutores"]->DetailEdit) {
						if (!isset($GLOBALS["tutores_grid"])) $GLOBALS["tutores_grid"] = new ctutores_grid(); // Get detail page object
						$EditRow = $GLOBALS["tutores_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("estado_actual_legajo_persona", $DetailTblVar)) {
				...
				if ($GLOBALS["estado_actual_legajo_persona_grid"]->DetailEdit) {
					$GLOBALS["estado_actual_legajo_persona_grid"]->CurrentMode = "edit";
					$GLOBALS["estado_actual_legajo_persona_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("materias_adeudadas", $DetailTblVar)) {
				...
				if ($GLOBALS["materias_adeudadas_grid"]->DetailEdit) {
					$GLOBALS["materias_adeudadas_grid"]->CurrentMode = "edit";
					$GLOBALS["materias_adeudadas_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("observacion_persona", $DetailTblVar)) {
				...
				if ($GLOBALS["observacion_persona_grid"]->DetailEdit) {
					$GLOBALS["observacion_persona_grid"]->CurrentMode = "edit";
					$GLOBALS["observacion_persona_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("equipos", $DetailTblVar)) {
				...
				if ($GLOBALS["equipos_grid"]->DetailEdit) {
					$GLOBALS["equipos_grid"]->CurrentMode = "edit";
					$GLOBALS["equipos_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("tutores", $DetailTblVar)) {
				...
				if ($GLOBALS["tutores_grid"]->DetailEdit) {
					$GLOBALS["tutores_grid"]->CurrentMode = "edit";
					$GLOBALS["tutores_grid"]->CurrentAction = "gridedit";
...
fpersonasedit.Lists["x_Repitente"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpersonasedit.Lists["x_Repitente"].Options = <?php echo json_encode($personas->Repitente->Options()) ?>;
fpersonasedit.Lists["x_Id_Sexo"] = {"LinkField":"x_Id_Sexo","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"sexo_personas"};
fpersonasedit.Lists["x_Id_Provincia"] = {"LinkField":"x_Id_Provincia","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":[],"ChildFields":["x_Id_Departamento"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"provincias"};
fpersonasedit.Lists["x_Id_Departamento"] = {"LinkField":"x_Id_Departamento","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":["x_Id_Provincia"],"ChildFields":["x_Id_Localidad"],"FilterFields":["x_Id_Provincia"],"Options":[],"Template":"","LinkTable":"departamento"};
fpersonasedit.Lists["x_Id_Localidad"] = {"LinkField":"x_Id_Localidad","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":["x_Id_Departamento"],"ChildFields":[],"FilterFields":["x_Id_Departamento"],"Options":[],"Template":"","LinkTable":"localidades"};
fpersonasedit.Lists["x_Id_Estado"] = {"LinkField":"x_Id_Estado","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"estado_persona"};
fpersonasedit.Lists["x_Id_Curso"] = {"LinkField":"x_Id_Curso","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cursos"};
fpersonasedit.Lists["x_Id_Division"] = {"LinkField":"x_Id_Division","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"division"};
fpersonasedit.Lists["x_Id_Turno"] = {"LinkField":"x_Id_Turno","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"turno"};
fpersonasedit.Lists["x_Id_Estado_Civil"] = {"LinkField":"x_Id_Estado_Civil","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"estado_civil"};
fpersonasedit.Lists["x_Id_Cargo"] = {"LinkField":"x_Id_Cargo","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cargo_persona"};
fpersonasedit.Lists["x_Dni_Tutor"] = {"LinkField":"x_Dni_Tutor","Ajax":true,"AutoFill":false,"DisplayFields":["x_Apellidos_Nombres","x_Dni_Tutor","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tutores"};
fpersonasedit.Lists["x_NroSerie"] = {"LinkField":"x_NroSerie","Ajax":true,"AutoFill":false,"DisplayFields":["x_NroSerie","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"equipos"};
...
<input type="hidden" data-table="personas" data-field="x_Dni" data-page="1" name="x_Dni" id="x_Dni" value="<?php echo ew_HtmlEncode($personas->Dni->CurrentValue) ?>">
...
<script type="text/javascript">
ew_CreateCalendar("fpersonasedit", "x_Fecha_Nac", 7);
</script>
<?php } ?>
</span>
...
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_NroSerie"><?php echo (strval($personas->NroSerie->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $personas->NroSerie->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($personas->NroSerie->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_NroSerie',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="personas" data-field="x_NroSerie" data-page="1" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $personas->NroSerie->DisplayValueSeparatorAttribute() ?>" name="x_NroSerie" id="x_NroSerie" value="<?php echo $personas->NroSerie->CurrentValue ?>"<?php echo $personas->NroSerie->EditAttributes() ?>>
...
<div class="panel-group" id="personas_edit_details">
<?php
	if (in_array("estado_actual_legajo_persona", explode(",", $personas->getCurrentDetailTable())) && $estado_actual_legajo_persona->DetailEdit) {
...
?>
	<div class="panel panel-default<?php echo $personas_edit->DetailPages->PageStyle("estado_actual_legajo_persona") ?>">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a class="panel-toggle" data-toggle="collapse" data-parent="#personas_edit_details" href="#tab_estado_actual_legajo_persona"><?php echo $Language->TablePhrase("estado_actual_legajo_persona", "TblCaption") ?></a>
			</h4>
		</div>
		<div class="panel-collapse collapse<?php echo $personas_edit->DetailPages->PageStyle("estado_actual_legajo_persona") ?>" id="tab_estado_actual_legajo_persona">
...
<?php } ?>
<?php
	if (in_array("materias_adeudadas", explode(",", $personas->getCurrentDetailTable())) && $materias_adeudadas->DetailEdit) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "materias_adeudadas") {
			$FirstActiveDetailTable = "materias_adeudadas";
		}
?>
	<div class="panel panel-default<?php echo $personas_edit->DetailPages->PageStyle("materias_adeudadas") ?>">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a class="panel-toggle" data-toggle="collapse" data-parent="#personas_edit_details" href="#tab_materias_adeudadas"><?php echo $Language->TablePhrase("materias_adeudadas", "TblCaption") ?></a>
			</h4>
		</div>
		<div class="panel-collapse collapse<?php echo $personas_edit->DetailPages->PageStyle("materias_adeudadas") ?>" id="tab_materias_adeudadas">
			<div class="panel-body">
...
<?php } ?>
<?php
	if (in_array("observacion_persona", explode(",", $personas->getCurrentDetailTable())) && $observacion_persona->DetailEdit) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "observacion_persona") {
			$FirstActiveDetailTable = "observacion_persona";
		}
?>
	<div class="panel panel-default<?php echo $personas_edit->DetailPages->PageStyle("observacion_persona") ?>">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a class="panel-toggle" data-toggle="collapse" data-parent="#personas_edit_details" href="#tab_observacion_persona"><?php echo $Language->TablePhrase("observacion_persona", "TblCaption") ?></a>
			</h4>
		</div>
		<div class="panel-collapse collapse<?php echo $personas_edit->DetailPages->PageStyle("observacion_persona") ?>" id="tab_observacion_persona">
			<div class="panel-body">
...
<?php } ?>
<?php
	if (in_array("equipos", explode(",", $personas->getCurrentDetailTable())) && $equipos->DetailEdit) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "equipos") {
			$FirstActiveDetailTable = "equipos";
		}
?>
	<div class="panel panel-default<?php echo $personas_edit->DetailPages->PageStyle("equipos") ?>">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a class="panel-toggle" data-toggle="collapse" data-parent="#personas_edit_details" href="#tab_equipos"><?php echo $Language->TablePhrase("equipos", "TblCaption") ?></a>
			</h4>
		</div>
		<div class="panel-collapse collapse<?php echo $personas_edit->DetailPages->PageStyle("equipos") ?>" id="tab_equipos">
			<div class="panel-body">
...
<?php } ?>
<?php
	if (in_array("tutores", explode(",", $personas->getCurrentDetailTable())) && $tutores->DetailEdit) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "tutores") {
			$FirstActiveDetailTable = "tutores";
		}
?>
	<div class="panel panel-default<?php echo $personas_edit->DetailPages->PageStyle("tutores") ?>">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a class="panel-toggle" data-toggle="collapse" data-parent="#personas_edit_details" href="#tab_tutores"><?php echo $Language->TablePhrase("tutores", "TblCaption") ?></a>
			</h4>
		</div>
		<div class="panel-collapse collapse<?php echo $personas_edit->DetailPages->PageStyle("tutores") ?>" id="tab_tutores">
			<div class="panel-body">
