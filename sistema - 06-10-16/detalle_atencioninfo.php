...
class cdetalle_atencion extends cTable {
	...
	function __construct() {
		...
		// Fecha_Actualizacion
		$this->Fecha_Actualizacion = new cField('detalle_atencion', 'detalle_atencion', 'x_Fecha_Actualizacion', 'Fecha_Actualizacion', '`Fecha_Actualizacion`', '`Fecha_Actualizacion`', 200, 7, FALSE, '`Fecha_Actualizacion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function SqlKeyFilter() {
		return "`Id_Atencion` = @Id_Atencion@ AND `Id_Detalle_Atencion` = @Id_Detalle_Atencion@";
	}
		...
		if (is_array($arKeys)) {
			foreach ($arKeys as $key) {
				...
				if (!is_numeric($key[0])) // Id_Atencion
					continue;
				if (!is_numeric($key[1])) // Id_Detalle_Atencion
					continue;
				$ar[] = $key;
			}
		}
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->Id_Detalle_Atencion->CurrentValue = $key[1];
