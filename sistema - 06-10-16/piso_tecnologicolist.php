...
class cpiso_tecnologico_list extends cpiso_tecnologico {
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->Id_Piso->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->Id_Piso->FormValue))
				return FALSE;
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->Id_Piso->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_Cantidad_Ap") && $objForm->HasValue("o_Cantidad_Ap") && $this->Cantidad_Ap->CurrentValue <> $this->Cantidad_Ap->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Estado_Ap") && $objForm->HasValue("o_Estado_Ap") && $this->Estado_Ap->CurrentValue <> $this->Estado_Ap->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Porcent_Estado_Ap") && $objForm->HasValue("o_Porcent_Estado_Ap") && $this->Porcent_Estado_Ap->CurrentValue <> $this->Porcent_Estado_Ap->OldValue)
			return FALSE;
	...
	function GetFilterList() {
		...
		$sFilterList = ew_Concat($sFilterList, $this->Porcent_Estado_Ap->AdvancedSearch->ToJSON(), ","); // Field Porcent_Estado_Ap
	...
	function RestoreFilterList() {
		...
		// Field Porcent_Estado_Ap
		$this->Porcent_Estado_Ap->AdvancedSearch->SearchValue = @$filter["x_Porcent_Estado_Ap"];
		$this->Porcent_Estado_Ap->AdvancedSearch->SearchOperator = @$filter["z_Porcent_Estado_Ap"];
		$this->Porcent_Estado_Ap->AdvancedSearch->SearchCondition = @$filter["v_Porcent_Estado_Ap"];
		$this->Porcent_Estado_Ap->AdvancedSearch->SearchValue2 = @$filter["y_Porcent_Estado_Ap"];
		$this->Porcent_Estado_Ap->AdvancedSearch->SearchOperator2 = @$filter["w_Porcent_Estado_Ap"];
		$this->Porcent_Estado_Ap->AdvancedSearch->Save();
	...
	function AdvancedSearchWhere($Default = FALSE) {
		...
		$this->BuildSearchSql($sWhere, $this->Estado_Ap, $Default, FALSE); // Estado_Ap
		$this->BuildSearchSql($sWhere, $this->Porcent_Estado_Ap, $Default, FALSE); // Porcent_Estado_Ap
		...
		if (!$Default && $this->Command == "search") {
			...
			$this->Estado_Ap->AdvancedSearch->Save(); // Estado_Ap
			$this->Porcent_Estado_Ap->AdvancedSearch->Save(); // Porcent_Estado_Ap
	...
	function CheckSearchParms() {
		...
		if ($this->Cantidad_Ap->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->Estado_Ap->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->Porcent_Estado_Ap->AdvancedSearch->IssetSession())
			return TRUE;
	...
	function ResetAdvancedSearchParms() {
		...
		$this->Estado_Ap->AdvancedSearch->UnsetSession();
		$this->Porcent_Estado_Ap->AdvancedSearch->UnsetSession();
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->Porcent_Estado_Ap); // Porcent_Estado_Ap
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				...
				$this->Porcent_Estado_Ap->setSort("");
				$this->Ups->setSort("");
	...
	function RenderListOptions() {
		...
		if ($Security->IsLoggedIn()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewCopy\" title=\"" . $copycaption . "\" data-table=\"piso_tecnologico\" data-caption=\"" . $copycaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,url:'" . ew_HtmlEncode($this->CopyUrl) . "',caption:'" . $copycaption . "'});\">" . $Language->Phrase("CopyLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->Id_Piso->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'>";
		if ($this->CurrentAction == "gridedit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->Id_Piso->CurrentValue . "\">";
		}
	...
	function SetupOtherOptions() {
		...
		if (ew_IsMobile())
			...
			$item->Body = "<a class=\"ewAddEdit ewAdd\" title=\"" . $addcaption . "\" data-table=\"piso_tecnologico\" data-caption=\"" . $addcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,url:'" . ew_HtmlEncode($this->AddUrl) . "',caption:'" . $addcaption . "'});\">" . $Language->Phrase("AddLink") . "</a>";
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.fpiso_tecnologicolist,url:'" . $this->MultiDeleteUrl . "'});return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->IsLoggedIn());
	...
	function LoadDefaultValues() {
		...
		$this->Estado_Ap->CurrentValue = NULL;
		$this->Estado_Ap->OldValue = $this->Estado_Ap->CurrentValue;
		$this->Porcent_Estado_Ap->CurrentValue = NULL;
		$this->Porcent_Estado_Ap->OldValue = $this->Porcent_Estado_Ap->CurrentValue;
		...
		$this->Ultima_Actualizacion->CurrentValue = NULL;
		$this->Ultima_Actualizacion->OldValue = $this->Ultima_Actualizacion->CurrentValue;
	...
	function LoadSearchValues() {
		...
		// Estado_Ap
		$this->Estado_Ap->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_Estado_Ap"]);
		if ($this->Estado_Ap->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->Estado_Ap->AdvancedSearch->SearchOperator = @$_GET["z_Estado_Ap"];

		// Porcent_Estado_Ap
		$this->Porcent_Estado_Ap->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_Porcent_Estado_Ap"]);
		if ($this->Porcent_Estado_Ap->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->Porcent_Estado_Ap->AdvancedSearch->SearchOperator = @$_GET["z_Porcent_Estado_Ap"];
	...
	function LoadFormValues() {
		...
		if (!$this->Estado_Ap->FldIsDetailKey) {
			$this->Estado_Ap->setFormValue($objForm->GetValue("x_Estado_Ap"));
		}
		$this->Estado_Ap->setOldValue($objForm->GetValue("o_Estado_Ap"));
		if (!$this->Porcent_Estado_Ap->FldIsDetailKey) {
			$this->Porcent_Estado_Ap->setFormValue($objForm->GetValue("x_Porcent_Estado_Ap"));
		}
		$this->Porcent_Estado_Ap->setOldValue($objForm->GetValue("o_Porcent_Estado_Ap"));
		...
		if (!$this->Ultima_Actualizacion->FldIsDetailKey) {
			$this->Ultima_Actualizacion->setFormValue($objForm->GetValue("x_Ultima_Actualizacion"));
			$this->Ultima_Actualizacion->CurrentValue = ew_UnFormatDateTime($this->Ultima_Actualizacion->CurrentValue, 7);
		}
		$this->Ultima_Actualizacion->setOldValue($objForm->GetValue("o_Ultima_Actualizacion"));
		if (!$this->Usuario->FldIsDetailKey) {
			$this->Usuario->setFormValue($objForm->GetValue("x_Usuario"));
		}
		$this->Usuario->setOldValue($objForm->GetValue("o_Usuario"));
		if (!$this->Id_Piso->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->Id_Piso->setFormValue($objForm->GetValue("x_Id_Piso"));
	}
	...
	function RestoreFormValues() {
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->Id_Piso->CurrentValue = $this->Id_Piso->FormValue;
		...
		$this->Estado_Ap->CurrentValue = $this->Estado_Ap->FormValue;
		$this->Porcent_Estado_Ap->CurrentValue = $this->Porcent_Estado_Ap->FormValue;
		...
		$this->Ultima_Actualizacion->CurrentValue = $this->Ultima_Actualizacion->FormValue;
		$this->Ultima_Actualizacion->CurrentValue = ew_UnFormatDateTime($this->Ultima_Actualizacion->CurrentValue, 7);
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("Id_Piso")) <> "")
			$this->Id_Piso->CurrentValue = $this->getKey("Id_Piso"); // Id_Piso
		else
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		$this->Usuario->ViewCustomAttributes = "";
			...
			if (trim(strval($this->Estado_Ap->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`Id_Estado_Equipo_piso`" . ew_SearchString("=", $this->Estado_Ap->CurrentValue, EW_DATATYPE_NUMBER, "");
			}
			...
			$this->Porcent_Estado_Ap->EditValue = ew_HtmlEncode($this->Porcent_Estado_Ap->CurrentValue);
	...
	function ValidateForm() {
		...
		if (!ew_CheckInteger($this->Porcent_Estado_Ap->FormValue)) {
			ew_AddMessage($gsFormError, $this->Porcent_Estado_Ap->FldErrMsg());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Estado_Ap
			$this->Estado_Ap->SetDbValueDef($rsnew, $this->Estado_Ap->CurrentValue, NULL, $this->Estado_Ap->ReadOnly);

			// Porcent_Estado_Ap
			$this->Porcent_Estado_Ap->SetDbValueDef($rsnew, $this->Porcent_Estado_Ap->CurrentValue, NULL, $this->Porcent_Estado_Ap->ReadOnly);
			...
			// Ultima_Actualizacion
			$this->Ultima_Actualizacion->SetDbValueDef($rsnew, ew_CurrentDate(), NULL);
			$rsnew['Ultima_Actualizacion'] = &$this->Ultima_Actualizacion->DbValue;
	...
	function AddRow($rsold = NULL) {
		...
		// Estado_Ap
		$this->Estado_Ap->SetDbValueDef($rsnew, $this->Estado_Ap->CurrentValue, NULL, FALSE);

		// Porcent_Estado_Ap
		$this->Porcent_Estado_Ap->SetDbValueDef($rsnew, $this->Porcent_Estado_Ap->CurrentValue, NULL, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->Id_Piso->setDbValue($conn->Insert_ID());
				$rsnew['Id_Piso'] = $this->Id_Piso->DbValue;
				if ($this->Plano_Escuela->Visible && !$this->Plano_Escuela->Upload->KeepFile) {
					$OldFiles = explode(EW_MULTIPLE_UPLOAD_SEPARATOR, $this->Plano_Escuela->Upload->DbValue);
					if (!ew_Empty($this->Plano_Escuela->Upload->FileName)) {
						$NewFiles = explode(EW_MULTIPLE_UPLOAD_SEPARATOR, $this->Plano_Escuela->Upload->FileName);
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a class=\"ewExportLink ewPrint\" title=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\" onclick=\"ew_Export(document.fpiso_tecnologicolist,'" . ew_CurrentPage() . "','print',false,true);\">" . $Language->Phrase("PrinterFriendly") . "</a>";
		$item->Visible = TRUE;

		// Export to Excel
		$item = &$this->ExportOptions->Add("excel");
		$item->Body = "<a class=\"ewExportLink ewExcel\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\" onclick=\"ew_Export(document.fpiso_tecnologicolist,'" . ew_CurrentPage() . "','excel',false,true);\">" . $Language->Phrase("ExportToExcel") . "</a>";
		$item->Visible = TRUE;

		// Export to Word
		$item = &$this->ExportOptions->Add("word");
		$item->Body = "<a class=\"ewExportLink ewWord\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\" onclick=\"ew_Export(document.fpiso_tecnologicolist,'" . ew_CurrentPage() . "','word',false,true);\">" . $Language->Phrase("ExportToWord") . "</a>";
		$item->Visible = TRUE;

		// Export to Html
		$item = &$this->ExportOptions->Add("html");
		$item->Body = "<a class=\"ewExportLink ewHtml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\" onclick=\"ew_Export(document.fpiso_tecnologicolist,'" . ew_CurrentPage() . "','html',false,true);\">" . $Language->Phrase("ExportToHtml") . "</a>";
		$item->Visible = FALSE;

		// Export to Xml
		$item = &$this->ExportOptions->Add("xml");
		$item->Body = "<a class=\"ewExportLink ewXml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\" onclick=\"ew_Export(document.fpiso_tecnologicolist,'" . ew_CurrentPage() . "','xml',false,true);\">" . $Language->Phrase("ExportToXml") . "</a>";
		$item->Visible = TRUE;

		// Export to Csv
		$item = &$this->ExportOptions->Add("csv");
		$item->Body = "<a class=\"ewExportLink ewCsv\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\" onclick=\"ew_Export(document.fpiso_tecnologicolist,'" . ew_CurrentPage() . "','csv',false,true);\">" . $Language->Phrase("ExportToCsv") . "</a>";
		$item->Visible = TRUE;

		// Export to Pdf
		$item = &$this->ExportOptions->Add("pdf");
		$item->Body = "<a class=\"ewExportLink ewPdf\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\" onclick=\"ew_Export(document.fpiso_tecnologicolist,'" . ew_CurrentPage() . "','pdf',false,true);\">" . $Language->Phrase("ExportToPDF") . "</a>";
		$item->Visible = FALSE;

		// Export to Email
		$item = &$this->ExportOptions->Add("email");
		$url = "";
		$item->Body = "<button id=\"emf_piso_tecnologico\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_piso_tecnologico',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fpiso_tecnologicolist,sel:true" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
fpiso_tecnologicolist.EmptyRow = function(infix) {
	...
	if (ew_ValueChanged(fobj, infix, "Estado_Ap", false)) return false;
	if (ew_ValueChanged(fobj, infix, "Porcent_Estado_Ap", false)) return false;
...
fpiso_tecnologicolist.Lists["x_Estado_Ap"] = {"LinkField":"x_Id_Estado_Equipo_piso","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"estado_equipos_piso"};
...
<?php if ($piso_tecnologico_list->TotalRecs > 0) { ?>
...
<?php if ($piso_tecnologico->Estado_Ap->Visible) { // Estado_Ap ?>
	<?php if ($piso_tecnologico->SortUrl($piso_tecnologico->Estado_Ap) == "") { ?>
		<th data-name="Estado_Ap"><div id="elh_piso_tecnologico_Estado_Ap" class="piso_tecnologico_Estado_Ap"><div class="ewTableHeaderCaption"><?php echo $piso_tecnologico->Estado_Ap->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Estado_Ap"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $piso_tecnologico->SortUrl($piso_tecnologico->Estado_Ap) ?>',1);"><div id="elh_piso_tecnologico_Estado_Ap" class="piso_tecnologico_Estado_Ap">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $piso_tecnologico->Estado_Ap->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($piso_tecnologico->Estado_Ap->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($piso_tecnologico->Estado_Ap->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($piso_tecnologico->Porcent_Estado_Ap->Visible) { // Porcent_Estado_Ap ?>
	<?php if ($piso_tecnologico->SortUrl($piso_tecnologico->Porcent_Estado_Ap) == "") { ?>
		<th data-name="Porcent_Estado_Ap"><div id="elh_piso_tecnologico_Porcent_Estado_Ap" class="piso_tecnologico_Porcent_Estado_Ap"><div class="ewTableHeaderCaption"><?php echo $piso_tecnologico->Porcent_Estado_Ap->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Porcent_Estado_Ap"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $piso_tecnologico->SortUrl($piso_tecnologico->Porcent_Estado_Ap) ?>',1);"><div id="elh_piso_tecnologico_Porcent_Estado_Ap" class="piso_tecnologico_Porcent_Estado_Ap">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $piso_tecnologico->Porcent_Estado_Ap->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($piso_tecnologico->Porcent_Estado_Ap->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($piso_tecnologico->Porcent_Estado_Ap->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
...
<?php if ($piso_tecnologico->Ultima_Actualizacion->Visible) { // Ultima_Actualizacion ?>
	<?php if ($piso_tecnologico->SortUrl($piso_tecnologico->Ultima_Actualizacion) == "") { ?>
		<th data-name="Ultima_Actualizacion"><div id="elh_piso_tecnologico_Ultima_Actualizacion" class="piso_tecnologico_Ultima_Actualizacion"><div class="ewTableHeaderCaption"><?php echo $piso_tecnologico->Ultima_Actualizacion->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Ultima_Actualizacion"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $piso_tecnologico->SortUrl($piso_tecnologico->Ultima_Actualizacion) ?>',1);"><div id="elh_piso_tecnologico_Ultima_Actualizacion" class="piso_tecnologico_Ultima_Actualizacion">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $piso_tecnologico->Ultima_Actualizacion->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($piso_tecnologico->Ultima_Actualizacion->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($piso_tecnologico->Ultima_Actualizacion->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
...
<a id="<?php echo $piso_tecnologico_list->PageObjName . "_row_" . $piso_tecnologico_list->RowCnt ?>"></a></td>
	<?php } ?>
<?php if ($piso_tecnologico->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="hidden" data-table="piso_tecnologico" data-field="x_Id_Piso" name="x<?php echo $piso_tecnologico_list->RowIndex ?>_Id_Piso" id="x<?php echo $piso_tecnologico_list->RowIndex ?>_Id_Piso" value="<?php echo ew_HtmlEncode($piso_tecnologico->Id_Piso->CurrentValue) ?>">
<input type="hidden" data-table="piso_tecnologico" data-field="x_Id_Piso" name="o<?php echo $piso_tecnologico_list->RowIndex ?>_Id_Piso" id="o<?php echo $piso_tecnologico_list->RowIndex ?>_Id_Piso" value="<?php echo ew_HtmlEncode($piso_tecnologico->Id_Piso->OldValue) ?>">
<?php } ?>
...
<?php } ?>
	...
	<?php } ?>
	<?php if ($piso_tecnologico->Estado_Ap->Visible) { // Estado_Ap ?>
		<td data-name="Estado_Ap"<?php echo $piso_tecnologico->Estado_Ap->CellAttributes() ?>>
<?php if ($piso_tecnologico->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $piso_tecnologico_list->RowCnt ?>_piso_tecnologico_Estado_Ap" class="form-group piso_tecnologico_Estado_Ap">
<select data-table="piso_tecnologico" data-field="x_Estado_Ap" data-value-separator="<?php echo $piso_tecnologico->Estado_Ap->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap" name="x<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap"<?php echo $piso_tecnologico->Estado_Ap->EditAttributes() ?>>
<?php echo $piso_tecnologico->Estado_Ap->SelectOptionListHtml("x<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap") ?>
</select>
<input type="hidden" name="s_x<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap" id="s_x<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap" value="<?php echo $piso_tecnologico->Estado_Ap->LookupFilterQuery() ?>">
</span>
<input type="hidden" data-table="piso_tecnologico" data-field="x_Estado_Ap" name="o<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap" id="o<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap" value="<?php echo ew_HtmlEncode($piso_tecnologico->Estado_Ap->OldValue) ?>">
<?php } ?>
<?php if ($piso_tecnologico->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $piso_tecnologico_list->RowCnt ?>_piso_tecnologico_Estado_Ap" class="form-group piso_tecnologico_Estado_Ap">
<select data-table="piso_tecnologico" data-field="x_Estado_Ap" data-value-separator="<?php echo $piso_tecnologico->Estado_Ap->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap" name="x<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap"<?php echo $piso_tecnologico->Estado_Ap->EditAttributes() ?>>
...
<?php } ?>
<?php if ($piso_tecnologico->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $piso_tecnologico_list->RowCnt ?>_piso_tecnologico_Estado_Ap" class="piso_tecnologico_Estado_Ap">
...
<?php } ?>
	...
	<?php } ?>
	<?php if ($piso_tecnologico->Porcent_Estado_Ap->Visible) { // Porcent_Estado_Ap ?>
		<td data-name="Porcent_Estado_Ap"<?php echo $piso_tecnologico->Porcent_Estado_Ap->CellAttributes() ?>>
<?php if ($piso_tecnologico->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $piso_tecnologico_list->RowCnt ?>_piso_tecnologico_Porcent_Estado_Ap" class="form-group piso_tecnologico_Porcent_Estado_Ap">
<input type="text" data-table="piso_tecnologico" data-field="x_Porcent_Estado_Ap" name="x<?php echo $piso_tecnologico_list->RowIndex ?>_Porcent_Estado_Ap" id="x<?php echo $piso_tecnologico_list->RowIndex ?>_Porcent_Estado_Ap" size="30" maxlength="3" placeholder="<?php echo ew_HtmlEncode($piso_tecnologico->Porcent_Estado_Ap->getPlaceHolder()) ?>" value="<?php echo $piso_tecnologico->Porcent_Estado_Ap->EditValue ?>"<?php echo $piso_tecnologico->Porcent_Estado_Ap->EditAttributes() ?>>
</span>
<input type="hidden" data-table="piso_tecnologico" data-field="x_Porcent_Estado_Ap" name="o<?php echo $piso_tecnologico_list->RowIndex ?>_Porcent_Estado_Ap" id="o<?php echo $piso_tecnologico_list->RowIndex ?>_Porcent_Estado_Ap" value="<?php echo ew_HtmlEncode($piso_tecnologico->Porcent_Estado_Ap->OldValue) ?>">
<?php } ?>
<?php if ($piso_tecnologico->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $piso_tecnologico_list->RowCnt ?>_piso_tecnologico_Porcent_Estado_Ap" class="form-group piso_tecnologico_Porcent_Estado_Ap">
<input type="text" data-table="piso_tecnologico" data-field="x_Porcent_Estado_Ap" name="x<?php echo $piso_tecnologico_list->RowIndex ?>_Porcent_Estado_Ap" id="x<?php echo $piso_tecnologico_list->RowIndex ?>_Porcent_Estado_Ap" size="30" maxlength="3" placeholder="<?php echo ew_HtmlEncode($piso_tecnologico->Porcent_Estado_Ap->getPlaceHolder()) ?>" value="<?php echo $piso_tecnologico->Porcent_Estado_Ap->EditValue ?>"<?php echo $piso_tecnologico->Porcent_Estado_Ap->EditAttributes() ?>>
...
<?php } ?>
<?php if ($piso_tecnologico->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $piso_tecnologico_list->RowCnt ?>_piso_tecnologico_Porcent_Estado_Ap" class="piso_tecnologico_Porcent_Estado_Ap">
...
<input type="hidden" name="fs_x<?php echo $piso_tecnologico_list->RowIndex ?>_Plano_Escuela" id= "fs_x<?php echo $piso_tecnologico_list->RowIndex ?>_Plano_Escuela" value="200">
...
<?php } ?>
	...
	<?php } ?>
	<?php if ($piso_tecnologico->Ultima_Actualizacion->Visible) { // Ultima_Actualizacion ?>
		<td data-name="Ultima_Actualizacion"<?php echo $piso_tecnologico->Ultima_Actualizacion->CellAttributes() ?>>
<?php if ($piso_tecnologico->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="hidden" data-table="piso_tecnologico" data-field="x_Ultima_Actualizacion" name="o<?php echo $piso_tecnologico_list->RowIndex ?>_Ultima_Actualizacion" id="o<?php echo $piso_tecnologico_list->RowIndex ?>_Ultima_Actualizacion" value="<?php echo ew_HtmlEncode($piso_tecnologico->Ultima_Actualizacion->OldValue) ?>">
<?php } ?>
<?php if ($piso_tecnologico->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<?php } ?>
<?php if ($piso_tecnologico->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $piso_tecnologico_list->RowCnt ?>_piso_tecnologico_Ultima_Actualizacion" class="piso_tecnologico_Ultima_Actualizacion">
...
<input type="hidden" data-table="piso_tecnologico" data-field="x_Cantidad_Ap" name="o<?php echo $piso_tecnologico_list->RowIndex ?>_Cantidad_Ap" id="o<?php echo $piso_tecnologico_list->RowIndex ?>_Cantidad_Ap" value="<?php echo ew_HtmlEncode($piso_tecnologico->Cantidad_Ap->OldValue) ?>">
	...
	<?php } ?>
	<?php if ($piso_tecnologico->Estado_Ap->Visible) { // Estado_Ap ?>
		<td data-name="Estado_Ap">
<span id="el$rowindex$_piso_tecnologico_Estado_Ap" class="form-group piso_tecnologico_Estado_Ap">
<select data-table="piso_tecnologico" data-field="x_Estado_Ap" data-value-separator="<?php echo $piso_tecnologico->Estado_Ap->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap" name="x<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap"<?php echo $piso_tecnologico->Estado_Ap->EditAttributes() ?>>
<?php echo $piso_tecnologico->Estado_Ap->SelectOptionListHtml("x<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap") ?>
</select>
<input type="hidden" name="s_x<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap" id="s_x<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap" value="<?php echo $piso_tecnologico->Estado_Ap->LookupFilterQuery() ?>">
...
<input type="hidden" data-table="piso_tecnologico" data-field="x_Estado_Ap" name="o<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap" id="o<?php echo $piso_tecnologico_list->RowIndex ?>_Estado_Ap" value="<?php echo ew_HtmlEncode($piso_tecnologico->Estado_Ap->OldValue) ?>">
	...
	<?php } ?>
	<?php if ($piso_tecnologico->Porcent_Estado_Ap->Visible) { // Porcent_Estado_Ap ?>
		<td data-name="Porcent_Estado_Ap">
<span id="el$rowindex$_piso_tecnologico_Porcent_Estado_Ap" class="form-group piso_tecnologico_Porcent_Estado_Ap">
<input type="text" data-table="piso_tecnologico" data-field="x_Porcent_Estado_Ap" name="x<?php echo $piso_tecnologico_list->RowIndex ?>_Porcent_Estado_Ap" id="x<?php echo $piso_tecnologico_list->RowIndex ?>_Porcent_Estado_Ap" size="30" maxlength="3" placeholder="<?php echo ew_HtmlEncode($piso_tecnologico->Porcent_Estado_Ap->getPlaceHolder()) ?>" value="<?php echo $piso_tecnologico->Porcent_Estado_Ap->EditValue ?>"<?php echo $piso_tecnologico->Porcent_Estado_Ap->EditAttributes() ?>>
</span>
<input type="hidden" data-table="piso_tecnologico" data-field="x_Porcent_Estado_Ap" name="o<?php echo $piso_tecnologico_list->RowIndex ?>_Porcent_Estado_Ap" id="o<?php echo $piso_tecnologico_list->RowIndex ?>_Porcent_Estado_Ap" value="<?php echo ew_HtmlEncode($piso_tecnologico->Porcent_Estado_Ap->OldValue) ?>">
</td>
...
<input type="hidden" data-table="piso_tecnologico" data-field="x_Porcent_Func_Piso" name="o<?php echo $piso_tecnologico_list->RowIndex ?>_Porcent_Func_Piso" id="o<?php echo $piso_tecnologico_list->RowIndex ?>_Porcent_Func_Piso" value="<?php echo ew_HtmlEncode($piso_tecnologico->Porcent_Func_Piso->OldValue) ?>">
	...
	<?php } ?>
	<?php if ($piso_tecnologico->Ultima_Actualizacion->Visible) { // Ultima_Actualizacion ?>
		<td data-name="Ultima_Actualizacion">
<input type="hidden" data-table="piso_tecnologico" data-field="x_Ultima_Actualizacion" name="o<?php echo $piso_tecnologico_list->RowIndex ?>_Ultima_Actualizacion" id="o<?php echo $piso_tecnologico_list->RowIndex ?>_Ultima_Actualizacion" value="<?php echo ew_HtmlEncode($piso_tecnologico->Ultima_Actualizacion->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($piso_tecnologico->Usuario->Visible) { // Usuario ?>
