...
$chat_update = NULL; // Initialize page object first

class cchat_update extends cchat {
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("chatlist.php"); // No records selected, return to list
	...
	function LoadMultiUpdateValues() {
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					$this->Texto_Chat->setDbValue($this->Recordset->fields('Texto_Chat'));
					$this->Hora->setDbValue($this->Recordset->fields('Hora'));
					$this->Usuario->setDbValue($this->Recordset->fields('Usuario'));
					$this->Estado->setDbValue($this->Recordset->fields('Estado'));
					$this->Nro_Conversacion->setDbValue($this->Recordset->fields('Nro_Conversacion'));
				} else {
					if (!ew_CompareValue($this->Texto_Chat->DbValue, $this->Recordset->fields('Texto_Chat')))
						$this->Texto_Chat->CurrentValue = NULL;
					if (!ew_CompareValue($this->Hora->DbValue, $this->Recordset->fields('Hora')))
					...
					if (!ew_CompareValue($this->Nro_Conversacion->DbValue, $this->Recordset->fields('Nro_Conversacion')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->Orden->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->Texto_Chat->MultiUpdate = $objForm->GetValue("u_Texto_Chat");
		...
		$this->Hora->MultiUpdate = $objForm->GetValue("u_Hora");
		...
		$this->Nro_Conversacion->MultiUpdate = $objForm->GetValue("u_Nro_Conversacion");
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($this->Texto_Chat->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->Hora->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->Usuario->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->Estado->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->Nro_Conversacion->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Texto_Chat
			$this->Texto_Chat->SetDbValueDef($rsnew, $this->Texto_Chat->CurrentValue, NULL, $this->Texto_Chat->ReadOnly || $this->Texto_Chat->MultiUpdate <> "1");
			...
			// Nro_Conversacion
			$this->Nro_Conversacion->SetDbValueDef($rsnew, $this->Nro_Conversacion->CurrentValue, 0, $this->Nro_Conversacion->ReadOnly || $this->Nro_Conversacion->MultiUpdate <> "1");
...
// Create page object
if (!isset($chat_update)) $chat_update = new cchat_update();

// Page init
$chat_update->Page_Init();

// Page main
$chat_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$chat_update->Page_Render();
?>
...
var CurrentForm = fchatupdate = new ew_Form("fchatupdate", "update");

// Validate form
fchatupdate.Validate = function() {
...
fchatupdate.Form_CustomValidate = 
...
fchatupdate.ValidateRequired = true;
<?php } else { ?>
fchatupdate.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $chat_update->ShowPageHeader(); ?>
<?php
$chat_update->ShowMessage();
?>
<form name="fchatupdate" id="fchatupdate" class="<?php echo $chat_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($chat_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $chat_update->Token ?>">
<?php } ?>
...
<?php } ?>
<?php foreach ($chat_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_chatupdate">
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($chat->Texto_Chat->Visible) { // Texto_Chat ?>
	<div id="r_Texto_Chat" class="form-group">
		<label for="x_Texto_Chat" class="col-sm-2 control-label">
<input type="checkbox" name="u_Texto_Chat" id="u_Texto_Chat" value="1"<?php echo ($chat->Texto_Chat->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $chat->Texto_Chat->FldCaption() ?></label>
...
<?php if ($chat->Hora->Visible) { // Hora ?>
	<div id="r_Hora" class="form-group">
		<label for="x_Hora" class="col-sm-2 control-label">
<input type="checkbox" name="u_Hora" id="u_Hora" value="1"<?php echo ($chat->Hora->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $chat->Hora->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $chat->Hora->CellAttributes() ?>>
<?php echo $chat->Hora->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($chat->Usuario->Visible) { // Usuario ?>
	<div id="r_Usuario" class="form-group">
		<label for="x_Usuario" class="col-sm-2 control-label">
<input type="checkbox" name="u_Usuario" id="u_Usuario" value="1"<?php echo ($chat->Usuario->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $chat->Usuario->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $chat->Usuario->CellAttributes() ?>>
<?php echo $chat->Usuario->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if (!$chat_update->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $chat_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div>
	</div>
<?php } ?>
</div>
