...
class cdenuncia_robo_equipo_list extends cdenuncia_robo_equipo {
	...
	function ClearInlineMode() {
		$this->setKey("IdDenuncia", ""); // Clear inline edit key
	...
	function InlineEditMode() {
		...
		if ($bInlineEdit) {
			if ($this->LoadRow()) {
				$this->setKey("IdDenuncia", $this->IdDenuncia->CurrentValue); // Set up inline edit key
	...
	function CheckInlineEditKey() {
		...
		if (strval($this->getKey("IdDenuncia")) <> strval($this->IdDenuncia->CurrentValue))
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentAction == "edit" && $this->RowType == EW_ROWTYPE_EDIT) { // Inline-Edit
			...
			$oListOpt->Body .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . ew_HtmlEncode($this->IdDenuncia->CurrentValue) . "\">";
			return;
		}
		...
		if ($Security->CanView()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewView\" title=\"" . $viewcaption . "\" data-table=\"denuncia_robo_equipo\" data-caption=\"" . $viewcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,url:'" . ew_HtmlEncode($this->ViewUrl) . "'});\">" . $Language->Phrase("ViewLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
		...
		if ($Security->CanEdit()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewEdit\" title=\"" . $editcaption . "\" data-table=\"denuncia_robo_equipo\" data-caption=\"" . $editcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,url:'" . ew_HtmlEncode($this->EditUrl) . "',caption:'" . $editcaption . "'});\">" . $Language->Phrase("EditLink") . "</a>";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.fdenuncia_robo_equipolist,url:'" . $this->MultiDeleteUrl . "',msg:ewLanguage.Phrase('DeleteConfirmMsg')});return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Fecha_Denuncia
			$this->Fecha_Denuncia->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->Fecha_Denuncia->CurrentValue, 7), NULL, $this->Fecha_Denuncia->ReadOnly);
...
fdenuncia_robo_equipolist.Lists["x_Dni"] = {"LinkField":"x_Dni","Ajax":true,"AutoFill":true,"DisplayFields":["x_Apellidos_Nombres","x_Dni","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"personas"};
fdenuncia_robo_equipolist.Lists["x_NroSerie"] = {"LinkField":"x_NroSerie","Ajax":true,"AutoFill":false,"DisplayFields":["x_NroSerie","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"equipos"};
fdenuncia_robo_equipolist.Lists["x_Id_Estado_Den"] = {"LinkField":"x_Id_Estado_Den","Ajax":true,"AutoFill":false,"DisplayFields":["x_Detalle","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"estado_denuncia"};
...
if ($denuncia_robo_equipo->CurrentAction == "gridadd") {
	...
	if ($denuncia_robo_equipo->CurrentAction == "" && $denuncia_robo_equipo_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$denuncia_robo_equipo_list->setWarningMessage(ew_DeniedMsg());
	...
	if ($denuncia_robo_equipo_list->AuditTrailOnSearch && $denuncia_robo_equipo_list->Command == "search" && !$denuncia_robo_equipo_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $denuncia_robo_equipo_list->getSessionWhere();
		$denuncia_robo_equipo_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
}
...
<?php if ($denuncia_robo_equipo_list->TotalRecs > 0 || $denuncia_robo_equipo->CurrentAction == "add" || $denuncia_robo_equipo->CurrentAction == "copy") { ?>
...
<?php if ($denuncia_robo_equipo->Id_Estado_Den->Visible) { // Id_Estado_Den ?>
	...
	<?php } else { ?>
		<th data-name="Id_Estado_Den"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $denuncia_robo_equipo->SortUrl($denuncia_robo_equipo->Id_Estado_Den) ?>',1);"><div id="elh_denuncia_robo_equipo_Id_Estado_Den" class="denuncia_robo_equipo_Id_Estado_Den">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $denuncia_robo_equipo->Id_Estado_Den->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($denuncia_robo_equipo->Id_Estado_Den->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($denuncia_robo_equipo->Id_Estado_Den->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($denuncia_robo_equipo->Usuario->Visible) { // Usuario ?>
	<?php if ($denuncia_robo_equipo->SortUrl($denuncia_robo_equipo->Usuario) == "") { ?>
		<th data-name="Usuario"><div id="elh_denuncia_robo_equipo_Usuario" class="denuncia_robo_equipo_Usuario"><div class="ewTableHeaderCaption"><?php echo $denuncia_robo_equipo->Usuario->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Usuario"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $denuncia_robo_equipo->SortUrl($denuncia_robo_equipo->Usuario) ?>',1);"><div id="elh_denuncia_robo_equipo_Usuario" class="denuncia_robo_equipo_Usuario">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $denuncia_robo_equipo->Usuario->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($denuncia_robo_equipo->Usuario->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($denuncia_robo_equipo->Usuario->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($denuncia_robo_equipo->Fecha_Actualizacion->Visible) { // Fecha_Actualizacion ?>
	<?php if ($denuncia_robo_equipo->SortUrl($denuncia_robo_equipo->Fecha_Actualizacion) == "") { ?>
		<th data-name="Fecha_Actualizacion"><div id="elh_denuncia_robo_equipo_Fecha_Actualizacion" class="denuncia_robo_equipo_Fecha_Actualizacion"><div class="ewTableHeaderCaption"><?php echo $denuncia_robo_equipo->Fecha_Actualizacion->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Fecha_Actualizacion"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $denuncia_robo_equipo->SortUrl($denuncia_robo_equipo->Fecha_Actualizacion) ?>',1);"><div id="elh_denuncia_robo_equipo_Fecha_Actualizacion" class="denuncia_robo_equipo_Fecha_Actualizacion">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $denuncia_robo_equipo->Fecha_Actualizacion->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($denuncia_robo_equipo->Fecha_Actualizacion->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($denuncia_robo_equipo->Fecha_Actualizacion->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
<tbody>
<?php
	if ($denuncia_robo_equipo->CurrentAction == "add" || $denuncia_robo_equipo->CurrentAction == "copy") {
		$denuncia_robo_equipo_list->RowIndex = 0;
		$denuncia_robo_equipo_list->KeyCount = $denuncia_robo_equipo_list->RowIndex;
		if ($denuncia_robo_equipo->CurrentAction == "add")
			$denuncia_robo_equipo_list->LoadDefaultValues();
		if ($denuncia_robo_equipo->EventCancelled) // Insert failed
			$denuncia_robo_equipo_list->RestoreFormValues(); // Restore form values

		// Set row properties
		$denuncia_robo_equipo->ResetAttrs();
		$denuncia_robo_equipo->RowAttrs = array_merge($denuncia_robo_equipo->RowAttrs, array('data-rowindex'=>0, 'id'=>'r0_denuncia_robo_equipo', 'data-rowtype'=>EW_ROWTYPE_ADD));
...
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Dni"><?php echo (strval($denuncia_robo_equipo->Dni->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $denuncia_robo_equipo->Dni->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($denuncia_robo_equipo->Dni->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Dni',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="denuncia_robo_equipo" data-field="x_Dni" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $denuncia_robo_equipo->Dni->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Dni" id="x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Dni" value="<?php echo $denuncia_robo_equipo->Dni->CurrentValue ?>"<?php echo $denuncia_robo_equipo->Dni->EditAttributes() ?>>
<?php if (AllowAdd(CurrentProjectID() . "personas")) { ?>
<button type="button" title="<?php echo ew_HtmlTitle($Language->Phrase("AddLink")) . "&nbsp;" . $denuncia_robo_equipo->Dni->FldCaption() ?>" onclick="ew_AddOptDialogShow({lnk:this,el:'x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Dni',url:'personasaddopt.php'});" class="ewAddOptBtn btn btn-default btn-sm" id="aol_x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Dni"><span class="glyphicon glyphicon-plus ewIcon"></span><span class="hide"><?php echo $Language->Phrase("AddLink") ?>&nbsp;<?php echo $denuncia_robo_equipo->Dni->FldCaption() ?></span></button>
<?php } ?>
<input type="hidden" name="s_x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Dni" id="s_x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Dni" value="<?php echo $denuncia_robo_equipo->Dni->LookupFilterQuery() ?>">
<input type="hidden" name="ln_x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Dni" id="ln_x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Dni" value="x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_NroSerie">
</span>
...
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_NroSerie"><?php echo (strval($denuncia_robo_equipo->NroSerie->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $denuncia_robo_equipo->NroSerie->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($denuncia_robo_equipo->NroSerie->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_NroSerie',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="denuncia_robo_equipo" data-field="x_NroSerie" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $denuncia_robo_equipo->NroSerie->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_NroSerie" id="x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_NroSerie" value="<?php echo $denuncia_robo_equipo->NroSerie->CurrentValue ?>"<?php echo $denuncia_robo_equipo->NroSerie->EditAttributes() ?>>
<?php if (AllowAdd(CurrentProjectID() . "equipos")) { ?>
<button type="button" title="<?php echo ew_HtmlTitle($Language->Phrase("AddLink")) . "&nbsp;" . $denuncia_robo_equipo->NroSerie->FldCaption() ?>" onclick="ew_AddOptDialogShow({lnk:this,el:'x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_NroSerie',url:'equiposaddopt.php'});" class="ewAddOptBtn btn btn-default btn-sm" id="aol_x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_NroSerie"><span class="glyphicon glyphicon-plus ewIcon"></span><span class="hide"><?php echo $Language->Phrase("AddLink") ?>&nbsp;<?php echo $denuncia_robo_equipo->NroSerie->FldCaption() ?></span></button>
<?php } ?>
<input type="hidden" name="s_x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_NroSerie" id="s_x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_NroSerie" value="<?php echo $denuncia_robo_equipo->NroSerie->LookupFilterQuery() ?>">
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fdenuncia_robo_equipolist", "x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Fecha_Denuncia", 7);
</script>
<?php } ?>
</span>
...
<select data-table="denuncia_robo_equipo" data-field="x_Id_Estado_Den" data-value-separator="<?php echo $denuncia_robo_equipo->Id_Estado_Den->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Id_Estado_Den" name="x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Id_Estado_Den"<?php echo $denuncia_robo_equipo->Id_Estado_Den->EditAttributes() ?>>
<?php echo $denuncia_robo_equipo->Id_Estado_Den->SelectOptionListHtml("x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Id_Estado_Den") ?>
</select>
<input type="hidden" name="s_x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Id_Estado_Den" id="s_x<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Id_Estado_Den" value="<?php echo $denuncia_robo_equipo->Id_Estado_Den->LookupFilterQuery() ?>">
</span>
...
<input type="hidden" data-table="denuncia_robo_equipo" data-field="x_Usuario" name="o<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Usuario" id="o<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Usuario" value="<?php echo ew_HtmlEncode($denuncia_robo_equipo->Usuario->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($denuncia_robo_equipo->Fecha_Actualizacion->Visible) { // Fecha_Actualizacion ?>
		<td data-name="Fecha_Actualizacion">
<input type="hidden" data-table="denuncia_robo_equipo" data-field="x_Fecha_Actualizacion" name="o<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Fecha_Actualizacion" id="o<?php echo $denuncia_robo_equipo_list->RowIndex ?>_Fecha_Actualizacion" value="<?php echo ew_HtmlEncode($denuncia_robo_equipo->Fecha_Actualizacion->OldValue) ?>">
</td>
	<?php } ?>
<?php
...
$denuncia_robo_equipo->ResetAttrs();
$denuncia_robo_equipo_list->RenderRow();
$denuncia_robo_equipo_list->EditRowCnt = 0;
if ($denuncia_robo_equipo->CurrentAction == "edit")
	$denuncia_robo_equipo_list->RowIndex = 1;
if ($denuncia_robo_equipo->CurrentAction == "gridadd")
	$denuncia_robo_equipo_list->RowIndex = 0;
...
while ($denuncia_robo_equipo_list->RecCnt < $denuncia_robo_equipo_list->StopRec) {
	...
	if (intval($denuncia_robo_equipo_list->RecCnt) >= intval($denuncia_robo_equipo_list->StartRec)) {
		...
		if ($denuncia_robo_equipo->CurrentAction == "edit") {
			if ($denuncia_robo_equipo_list->CheckInlineEditKey() && $denuncia_robo_equipo_list->EditRowCnt == 0) { // Inline edit
		...
		if ($denuncia_robo_equipo->CurrentAction == "edit" && $denuncia_robo_equipo->RowType == EW_ROWTYPE_EDIT && $denuncia_robo_equipo->EventCancelled) { // Update failed
			$objForm->Index = 1;
			$denuncia_robo_equipo_list->RestoreFormValues(); // Restore form values
		}
...
<?php } ?>
<?php if ($denuncia_robo_equipo->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $denuncia_robo_equipo_list->RowCnt ?>_denuncia_robo_equipo_Usuario" class="denuncia_robo_equipo_Usuario">
...
<?php } ?>
<?php if ($denuncia_robo_equipo->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $denuncia_robo_equipo_list->RowCnt ?>_denuncia_robo_equipo_Fecha_Actualizacion" class="denuncia_robo_equipo_Fecha_Actualizacion">
...
<?php } ?>
<?php if ($denuncia_robo_equipo->CurrentAction == "add" || $denuncia_robo_equipo->CurrentAction == "copy") { ?>
...
<?php } ?>
<?php if ($denuncia_robo_equipo->CurrentAction == "edit") { ?>
<input type="hidden" name="<?php echo $denuncia_robo_equipo_list->FormKeyCountName ?>" id="<?php echo $denuncia_robo_equipo_list->FormKeyCountName ?>" value="<?php echo $denuncia_robo_equipo_list->KeyCount ?>">
<?php } ?>
