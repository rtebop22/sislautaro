...
$detalle_asistencia_view = NULL; // Initialize page object first

class cdetalle_asistencia_view extends cdetalle_asistencia {
	...
	// Page object name
	var $PageObjName = 'detalle_asistencia_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["Dia"] <> "") {
			$this->RecKey["Dia"] = $_GET["Dia"];
			$KeyUrl .= "&amp;Dia=" . urlencode($this->RecKey["Dia"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["Dia"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["Dia"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["Dia"] <> "") {
				...
				$this->RecKey["Dia"] = $this->Dia->QueryStringValue;
			} elseif (@$_POST["Dia"] <> "") {
				$this->Dia->setFormValue($_POST["Dia"]);
				$this->RecKey["Dia"] = $this->Dia->FormValue;
			} else {
				$sReturnUrl = "detalle_asistencialist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "detalle_asistencialist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "detalle_asistencialist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($detalle_asistencia_view)) $detalle_asistencia_view = new cdetalle_asistencia_view();

// Page init
$detalle_asistencia_view->Page_Init();

// Page main
$detalle_asistencia_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$detalle_asistencia_view->Page_Render();
?>
...
var CurrentForm = fdetalle_asistenciaview = new ew_Form("fdetalle_asistenciaview", "view");

// Form_CustomValidate event
fdetalle_asistenciaview.Form_CustomValidate = 
...
fdetalle_asistenciaview.ValidateRequired = true;
<?php } else { ?>
fdetalle_asistenciaview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdetalle_asistenciaview.Lists["x_Dia"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fdetalle_asistenciaview.Lists["x_Dia"].Options = <?php echo json_encode($detalle_asistencia->Dia->Options()) ?>;
...
<?php if ($detalle_asistencia->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$detalle_asistencia_view->IsModal) { ?>
<?php if ($detalle_asistencia->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $detalle_asistencia_view->ExportOptions->Render("body") ?>
<?php
	foreach ($detalle_asistencia_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$detalle_asistencia_view->IsModal) { ?>
<?php if ($detalle_asistencia->Export == "") { ?>
...
<?php } ?>
<?php $detalle_asistencia_view->ShowPageHeader(); ?>
<?php
$detalle_asistencia_view->ShowMessage();
?>
<form name="fdetalle_asistenciaview" id="fdetalle_asistenciaview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($detalle_asistencia_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $detalle_asistencia_view->Token ?>">
<?php } ?>
...
<?php if ($detalle_asistencia->Dia->Visible) { // Dia ?>
	<tr id="r_Dia">
		<td><span id="elh_detalle_asistencia_Dia"><?php echo $detalle_asistencia->Dia->FldCaption() ?></span></td>
...
<?php echo $detalle_asistencia->Dia->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($detalle_asistencia->Dias->Visible) { // Dias ?>
	<tr id="r_Dias">
		<td><span id="elh_detalle_asistencia_Dias"><?php echo $detalle_asistencia->Dias->FldCaption() ?></span></td>
...
<?php echo $detalle_asistencia->Dias->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($detalle_asistencia->Horario->Visible) { // Horario ?>
	<tr id="r_Horario">
		<td><span id="elh_detalle_asistencia_Horario"><?php echo $detalle_asistencia->Horario->FldCaption() ?></span></td>
...
<?php echo $detalle_asistencia->Horario->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($detalle_asistencia->Rol->Visible) { // Rol ?>
	<tr id="r_Rol">
		<td><span id="elh_detalle_asistencia_Rol"><?php echo $detalle_asistencia->Rol->FldCaption() ?></span></td>
...
<?php echo $detalle_asistencia->Rol->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($detalle_asistencia->Observacion->Visible) { // Observacion ?>
	<tr id="r_Observacion">
		<td><span id="elh_detalle_asistencia_Observacion"><?php echo $detalle_asistencia->Observacion->FldCaption() ?></span></td>
...
<?php echo $detalle_asistencia->Observacion->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($detalle_asistencia->Id_Asistencia->Visible) { // Id_Asistencia ?>
	<tr id="r_Id_Asistencia">
		<td><span id="elh_detalle_asistencia_Id_Asistencia"><?php echo $detalle_asistencia->Id_Asistencia->FldCaption() ?></span></td>
...
<?php echo $detalle_asistencia->Id_Asistencia->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($detalle_asistencia->Export == "") { ?>
<script type="text/javascript">
fdetalle_asistenciaview.Init();
</script>
<?php } ?>
<?php
$detalle_asistencia_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($detalle_asistencia->Export == "") { ?>
...
<?php
$detalle_asistencia_view->Page_Terminate();
?>
