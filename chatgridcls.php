...
$chat_grid = NULL; // Initialize page object first

class cchat_grid extends cchat {
	...
	// Page object name
	var $PageObjName = 'chat_grid';

	// Grid form hidden field names
	var $FormName = 'fchatgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["chat"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->Orden->CurrentValue = strval($arKeys[0]); // Orden
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "conversaciones") {
			$this->Nro_Conversacion->Visible = FALSE;
			if ($GLOBALS["conversaciones"]->EventCancelled) $this->EventCancelled = TRUE;
		}
