...
$tutores_grid = NULL; // Initialize page object first

class ctutores_grid extends ctutores {
	...
	// Page object name
	var $PageObjName = 'tutores_grid';

	// Grid form hidden field names
	var $FormName = 'ftutoresgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["tutores"];

		}
		$this->AddUrl = "tutoresadd.php";
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->Dni_Tutor->CurrentValue = strval($arKeys[0]); // Dni_Tutor
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
