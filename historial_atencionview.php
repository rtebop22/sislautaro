...
$historial_atencion_view = NULL; // Initialize page object first

class chistorial_atencion_view extends chistorial_atencion {
	...
	// Page object name
	var $PageObjName = 'historial_atencion_view';
	...
	function __construct() {
		...
		if (@$_GET["Id_Historial"] <> "") {
			$this->RecKey["Id_Historial"] = $_GET["Id_Historial"];
			$KeyUrl .= "&amp;Id_Historial=" . urlencode($this->RecKey["Id_Historial"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["Id_Historial"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["Id_Historial"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["Id_Historial"] <> "") {
				...
				$this->RecKey["Id_Historial"] = $this->Id_Historial->QueryStringValue;
			} elseif (@$_POST["Id_Historial"] <> "") {
				$this->Id_Historial->setFormValue($_POST["Id_Historial"]);
				$this->RecKey["Id_Historial"] = $this->Id_Historial->FormValue;
			} else {
				$sReturnUrl = "historial_atencionlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "historial_atencionlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "historial_atencionlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($historial_atencion_view)) $historial_atencion_view = new chistorial_atencion_view();

// Page init
$historial_atencion_view->Page_Init();

// Page main
$historial_atencion_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$historial_atencion_view->Page_Render();
?>
...
var CurrentForm = fhistorial_atencionview = new ew_Form("fhistorial_atencionview", "view");

// Form_CustomValidate event
fhistorial_atencionview.Form_CustomValidate = 
...
fhistorial_atencionview.ValidateRequired = true;
<?php } else { ?>
fhistorial_atencionview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($historial_atencion->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$historial_atencion_view->IsModal) { ?>
<?php if ($historial_atencion->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $historial_atencion_view->ExportOptions->Render("body") ?>
<?php
	foreach ($historial_atencion_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$historial_atencion_view->IsModal) { ?>
<?php if ($historial_atencion->Export == "") { ?>
...
<?php } ?>
<?php $historial_atencion_view->ShowPageHeader(); ?>
<?php
$historial_atencion_view->ShowMessage();
?>
<form name="fhistorial_atencionview" id="fhistorial_atencionview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($historial_atencion_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $historial_atencion_view->Token ?>">
<?php } ?>
...
<?php if ($historial_atencion->Id_Historial->Visible) { // Id_Historial ?>
	<tr id="r_Id_Historial">
		<td><span id="elh_historial_atencion_Id_Historial"><?php echo $historial_atencion->Id_Historial->FldCaption() ?></span></td>
		<td data-name="Id_Historial"<?php echo $historial_atencion->Id_Historial->CellAttributes() ?>>
<span id="el_historial_atencion_Id_Historial">
<span<?php echo $historial_atencion->Id_Historial->ViewAttributes() ?>>
<?php echo $historial_atencion->Id_Historial->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($historial_atencion->Detalle->Visible) { // Detalle ?>
	<tr id="r_Detalle">
		<td><span id="elh_historial_atencion_Detalle"><?php echo $historial_atencion->Detalle->FldCaption() ?></span></td>
...
<?php echo $historial_atencion->Detalle->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($historial_atencion->Fecha_Actualizacion->Visible) { // Fecha_Actualizacion ?>
	<tr id="r_Fecha_Actualizacion">
		<td><span id="elh_historial_atencion_Fecha_Actualizacion"><?php echo $historial_atencion->Fecha_Actualizacion->FldCaption() ?></span></td>
...
<?php echo $historial_atencion->Fecha_Actualizacion->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($historial_atencion->Usuario->Visible) { // Usuario ?>
	<tr id="r_Usuario">
		<td><span id="elh_historial_atencion_Usuario"><?php echo $historial_atencion->Usuario->FldCaption() ?></span></td>
...
<?php echo $historial_atencion->Usuario->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($historial_atencion->Export == "") { ?>
<script type="text/javascript">
fhistorial_atencionview.Init();
</script>
<?php } ?>
<?php
$historial_atencion_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($historial_atencion->Export == "") { ?>
...
<?php
$historial_atencion_view->Page_Terminate();
?>
