...
class catencion_para_st_update extends catencion_para_st {
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->Referencia_Tipo_Retiro->setDbValue($this->Recordset->fields('Referencia_Tipo_Retiro'));
					...
					if (!ew_CompareValue($this->Referencia_Tipo_Retiro->DbValue, $this->Recordset->fields('Referencia_Tipo_Retiro')))
	...
	function LoadFormValues() {
		...
		$this->Referencia_Tipo_Retiro->MultiUpdate = $objForm->GetValue("u_Referencia_Tipo_Retiro");
	...
	function ValidateForm() {
		...
		if ($this->Referencia_Tipo_Retiro->MultiUpdate == "1") $lUpdateCnt++;
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($this->Nro_Tiket->MultiUpdate <> "" && !$this->Nro_Tiket->FldIsDetailKey && !is_null($this->Nro_Tiket->FormValue) && $this->Nro_Tiket->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Referencia_Tipo_Retiro
			$this->Referencia_Tipo_Retiro->SetDbValueDef($rsnew, $this->Referencia_Tipo_Retiro->CurrentValue, NULL, $this->Referencia_Tipo_Retiro->ReadOnly || $this->Referencia_Tipo_Retiro->MultiUpdate <> "1");

			// Fecha_Retiro
			$this->Fecha_Retiro->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->Fecha_Retiro->CurrentValue, 7), NULL, $this->Fecha_Retiro->ReadOnly || $this->Fecha_Retiro->MultiUpdate <> "1");
			...
			// Fecha_Devolucion
			$this->Fecha_Devolucion->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->Fecha_Devolucion->CurrentValue, 7), NULL, $this->Fecha_Devolucion->ReadOnly || $this->Fecha_Devolucion->MultiUpdate <> "1");
...
<?php if ($atencion_para_st->Referencia_Tipo_Retiro->Visible) { // Referencia_Tipo_Retiro ?>
	<div id="r_Referencia_Tipo_Retiro" class="form-group">
		<label for="x_Referencia_Tipo_Retiro" class="col-sm-2 control-label">
<input type="checkbox" name="u_Referencia_Tipo_Retiro" id="u_Referencia_Tipo_Retiro" value="1"<?php echo ($atencion_para_st->Referencia_Tipo_Retiro->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $atencion_para_st->Referencia_Tipo_Retiro->FldCaption() ?></label>
