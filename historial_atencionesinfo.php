...
$historial_atenciones = NULL;

//
// Table class for historial_atenciones
//
class chistorial_atenciones extends cTable {
	var $Id_Atencion;
	...
	function __construct() {
		...
		$this->TableVar = 'historial_atenciones';
		$this->TableName = 'historial_atenciones';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`historial_atenciones`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// Id_Atencion
		$this->Id_Atencion = new cField('historial_atenciones', 'historial_atenciones', 'x_Id_Atencion', 'Id_Atencion', '`Id_Atencion`', '`Id_Atencion`', 3, -1, FALSE, '`Id_Atencion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		...
		// Apellidos_Nombres
		$this->Apellidos_Nombres = new cField('historial_atenciones', 'historial_atenciones', 'x_Apellidos_Nombres', 'Apellidos_Nombres', '`Apellidos_Nombres`', '`Apellidos_Nombres`', 201, -1, FALSE, '`Apellidos_Nombres`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Apellidos_Nombres->Sortable = TRUE; // Allow sort
		$this->fields['Apellidos_Nombres'] = &$this->Apellidos_Nombres;

		// Dni
		$this->Dni = new cField('historial_atenciones', 'historial_atenciones', 'x_Dni', 'Dni', '`Dni`', '`Dni`', 3, -1, FALSE, '`Dni`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Dni->Sortable = TRUE; // Allow sort
		$this->Dni->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Dni'] = &$this->Dni;

		// Detalle
		$this->Detalle = new cField('historial_atenciones', 'historial_atenciones', 'x_Detalle', 'Detalle', '`Detalle`', '`Detalle`', 201, -1, FALSE, '`Detalle`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Detalle->Sortable = TRUE; // Allow sort
		$this->fields['Detalle'] = &$this->Detalle;

		// Fecha_Actualizacion
		$this->Fecha_Actualizacion = new cField('historial_atenciones', 'historial_atenciones', 'x_Fecha_Actualizacion', 'Fecha_Actualizacion', '`Fecha_Actualizacion`', 'DATE_FORMAT(`Fecha_Actualizacion`, \'\')', 133, 7, FALSE, '`Fecha_Actualizacion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Usuario
		$this->Usuario = new cField('historial_atenciones', 'historial_atenciones', 'x_Usuario', 'Usuario', '`Usuario`', '`Usuario`', 200, -1, FALSE, '`Usuario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`historial_atenciones`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "historial_atencioneslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("historial_atencionesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("historial_atencionesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "historial_atencionesadd.php?" . $this->UrlParm($parm);
		else
			$url = "historial_atencionesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("historial_atencionesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("historial_atencionesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("historial_atencionesdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	$Fecha_Devol=$rsnew["Fecha_Devolucion"];
	if ($Retiro==1){
