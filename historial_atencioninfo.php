...
$historial_atencion = NULL;

//
// Table class for historial_atencion
//
class chistorial_atencion extends cTable {
	var $Id_Historial;
	var $Detalle;
	...
	function __construct() {
		...
		$this->TableVar = 'historial_atencion';
		$this->TableName = 'historial_atencion';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`historial_atencion`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// Id_Historial
		$this->Id_Historial = new cField('historial_atencion', 'historial_atencion', 'x_Id_Historial', 'Id_Historial', '`Id_Historial`', '`Id_Historial`', 3, -1, FALSE, '`Id_Historial`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->Id_Historial->Sortable = TRUE; // Allow sort
		$this->Id_Historial->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Id_Historial'] = &$this->Id_Historial;

		// Detalle
		$this->Detalle = new cField('historial_atencion', 'historial_atencion', 'x_Detalle', 'Detalle', '`Detalle`', '`Detalle`', 201, -1, FALSE, '`Detalle`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Detalle->Sortable = TRUE; // Allow sort
		$this->fields['Detalle'] = &$this->Detalle;

		// Fecha_Actualizacion
		$this->Fecha_Actualizacion = new cField('historial_atencion', 'historial_atencion', 'x_Fecha_Actualizacion', 'Fecha_Actualizacion', '`Fecha_Actualizacion`', 'DATE_FORMAT(`Fecha_Actualizacion`, \'\')', 133, 7, FALSE, '`Fecha_Actualizacion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Usuario
		$this->Usuario = new cField('historial_atencion', 'historial_atencion', 'x_Usuario', 'Usuario', '`Usuario`', '`Usuario`', 200, -1, FALSE, '`Usuario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Usuario->Sortable = TRUE; // Allow sort
		$this->fields['Usuario'] = &$this->Usuario;

		// Id_Atencion
		$this->Id_Atencion = new cField('historial_atencion', 'historial_atencion', 'x_Id_Atencion', 'Id_Atencion', '`Id_Atencion`', '`Id_Atencion`', 3, -1, FALSE, '`Id_Atencion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		...
		// NroSerie
		$this->NroSerie = new cField('historial_atencion', 'historial_atencion', 'x_NroSerie', 'NroSerie', '`NroSerie`', '`NroSerie`', 200, -1, FALSE, '`NroSerie`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`historial_atencion`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('Id_Historial', $rs))
				ew_AddFilter($where, ew_QuotedName('Id_Historial', $this->DBID) . '=' . ew_QuotedValue($rs['Id_Historial'], $this->Id_Historial->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@Id_Historial@", ew_AdjustSql($this->Id_Historial->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "historial_atencionlist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("historial_atencionview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("historial_atencionview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "historial_atencionadd.php?" . $this->UrlParm($parm);
		else
			$url = "historial_atencionadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("historial_atencionedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("historial_atencionadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("historial_atenciondelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "Id_Historial:" . ew_VarToJson($this->Id_Historial->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["Id_Historial"]))
				$arKeys[] = ew_StripSlashes($_POST["Id_Historial"]);
			elseif (isset($_GET["Id_Historial"]))
				$arKeys[] = ew_StripSlashes($_GET["Id_Historial"]);
			else
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->Id_Historial->Exportable) $Doc->ExportCaption($this->Id_Historial);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->Id_Historial->Exportable) $Doc->ExportField($this->Id_Historial);
